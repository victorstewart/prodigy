...
static inline bool prodigyMachineProvisioningReady(const Machine& machine)
{
  IPAddress peerAddress = {};
  String sshAddress = {};
  return prodigyResolveMachinePeerAddress(machine, peerAddress) && prodigyResolveMachineSSHAddress(machine, sshAddress);
}
...
{
  return prodigyMachineProvisioningReady(machine) && prodigyMachineSSHSocketAcceptingConnections(machine, timeoutMs);
}
...
static inline bool prodigyMachinesShareIdentity(const Machine& lhs, const Machine& rhs)
{
  ...
  if (lhs.cloudID.size() > 0 && rhs.cloudID.size() > 0 && lhs.cloudID.equals(rhs.cloudID))
  {
    return true;
  }
  ...
  if (prodigyResolveMachineSSHAddress(lhs, lhsSSHAddress) && prodigyResolveMachineSSHAddress(rhs, rhsSSHAddress) && lhsSSHAddress.equals(rhsSSHAddress))
  {
    return true;
  }

  return (lhsHasPeerAddress == false || rhsHasPeerAddress == false) && lhs.private4 != 0 && rhs.private4 != 0 && lhs.private4 == rhs.private4;
}
...
static inline void prodigyBackfillClusterMachineFromAuthoritativeRecord(ClusterMachine& machine, const ClusterMachine& authoritative)
{
  ...
  bool machineOwnershipDefault =
      machine.ownership.mode == ClusterMachineOwnershipMode::wholeMachine && machine.ownership.nLogicalCoresCap == 0 && machine.ownership.nMemoryMBCap == 0 && machine.ownership.nStorageMBCap == 0 && machine.ownership.nLogicalCoresBasisPoints == 0 && machine.ownership.nMemoryBasisPoints == 0 && machine.ownership.nStorageBasisPoints == 0;
  bool authoritativeOwnershipSpecified =
      authoritative.ownership.mode != ClusterMachineOwnershipMode::wholeMachine || authoritative.ownership.nLogicalCoresCap > 0 || authoritative.ownership.nMemoryMBCap > 0 || authoritative.ownership.nStorageMBCap > 0 || authoritative.ownership.nLogicalCoresBasisPoints > 0 || authoritative.ownership.nMemoryBasisPoints > 0 || authoritative.ownership.nStorageBasisPoints > 0;
...
static inline bool prodigyClusterMachineBootstrapReady(const ClusterMachine& clusterMachine)
{
  ...
  return sshAddress.size() > 0 && clusterMachine.ssh.user.size() > 0 && clusterMachine.ssh.privateKeyPath.size() > 0;
}
...
static inline bool prodigyClusterMachineMatchesMachineIdentity(const ClusterMachine& clusterMachine, const Machine& machine)
{
  ...
  if (clusterMachine.ssh.address.size() > 0 && machine.sshAddress.size() > 0 && clusterMachine.ssh.address.equals(machine.sshAddress))
  {
    return true;
  }
  ...
  return (clusterHasPeerAddress == false || machineHasPeerAddress == false) && clusterMachine.resolvePrivate4(clusterPrivate4) && machine.private4 != 0 && clusterPrivate4 == machine.private4;
}
...
{
  return hardware.inventoryComplete && hardware.cpu.logicalCores > 0 && hardware.memory.totalMB > 0;
}
...
static inline bool prodigyMachineReadyResourcesAvailable(const Machine& machine)
{
  ...
  return readyLogicalCores > 0 && readyMemoryMB > 0 && readyStorageMB > 0;
}
