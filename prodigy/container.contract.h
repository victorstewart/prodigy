...
static constexpr uint32_t prodigyDiscombobulatorAppContractVersion = 1;
   ...
   String *failureReport = nullptr)
{
   ...
   {
      if (failureReport) failureReport->assign("container blob is missing the supported Discombobulator app-container contract header"_ctv);
      return false;
   }
   ...
   String *failureReport = nullptr)
{
   ...
   if (openedFD < 0)
   {
      if (failureReport) failureReport->snprintf<"failed to open container blob {} errno={}({})"_ctv>(blobPath, errno, String(strerror(errno)));
      return false;
   }
   ...
   while (readBytes < expectedHeader.size())
   {
      ...
      if (result == 0)
      {
         if (failureReport) failureReport->assign("container blob is missing the supported Discombobulator app-container contract header: ended before header completed"_ctv);
         close(openedFD);
         return false;
      }
      ...
      if (failureReport) failureReport->snprintf<"failed to read container blob contract header errno={}({})"_ctv>(errno, String(strerror(errno)));
      close(openedFD);
      return false;
   }
