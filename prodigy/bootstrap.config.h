...
public:
  ...
  {
    if (nodeRole != other.nodeRole || controlSocketPath.equals(other.controlSocketPath) == false || bootstrapPeers.size() != other.bootstrapPeers.size())
    {
      return false;
    }
...
static inline bool parseProdigyBootstrapPeerJSONElement(simdjson::dom::element element, ProdigyBootstrapConfig::BootstrapPeer& peer, String *failure = nullptr)
{
  if (element.type() != simdjson::dom::element_type::OBJECT)
  {
    if (failure)
    {
      failure->assign("bootstrapPeers requires peer objects");
    }
    return false;
  }
  ...
  for (auto field : element.get_object())
  {
    ...
    if (key.equal("isBrain"_ctv))
    {
      if (field.value.type() != simdjson::dom::element_type::BOOL)
      {
        if (failure)
        {
          failure->assign("bootstrapPeers isBrain requires bool");
        }
        return false;
      }
      ...
      if (field.value.get(value) != simdjson::SUCCESS)
      {
        if (failure)
        {
          failure->assign("bootstrapPeers isBrain invalid");
        }
        return false;
      }
    ...
    else if (key.equal("addresses"_ctv))
    {
      if (field.value.type() != simdjson::dom::element_type::ARRAY)
      {
        if (failure)
        {
          failure->assign("bootstrapPeers addresses requires array");
        }
        return false;
      }
      ...
      for (auto candidateElement : field.value.get_array())
      {
        if (candidateElement.type() != simdjson::dom::element_type::OBJECT)
        {
          if (failure)
          {
            failure->assign("bootstrapPeers addresses requires candidate objects");
          }
          return false;
        }
        ...
        for (auto candidateField : candidateElement.get_object())
        {
          ...
          if (candidateKey.equal("address"_ctv))
          {
            if (candidateField.value.type() != simdjson::dom::element_type::STRING)
            {
              if (failure)
              {
                failure->assign("bootstrapPeers address requires string");
              }
              return false;
            }
          ...
          {
            if (candidateField.value.type() != simdjson::dom::element_type::INT64 && candidateField.value.type() != simdjson::dom::element_type::UINT64)
            {
              if (failure)
              {
                failure->assign("bootstrapPeers cidr requires integer");
              }
              return false;
            }
            ...
            if (candidateField.value.type() == simdjson::dom::element_type::INT64)
            {
              ...
              if (candidateField.value.get_int64().get(signedCidr) || signedCidr < 0)
              {
                if (failure)
                {
                  failure->assign("bootstrapPeers cidr invalid");
                }
                return false;
              }
          ...
          else
          {
            if (failure)
            {
              failure->assign("invalid bootstrap peer candidate field");
            }
            return false;
          }
        }
        ...
        if (sawAddress == false)
        {
          if (failure)
          {
            failure->assign("bootstrapPeers candidate address required");
          }
          return false;
        }
    ...
    else
    {
      if (failure)
      {
        failure->assign("invalid bootstrap peer field");
      }
      return false;
    }
  }
  ...
  if (sawAddresses == false)
  {
    if (failure)
    {
      failure->assign("bootstrapPeers addresses required");
    }
    return false;
  }
  ...
  if (parsed.addresses.empty())
  {
    if (failure)
    {
      failure->assign("bootstrapPeers addresses cannot be empty");
    }
    return false;
  }
...
static inline bool parseProdigyBootstrapConfigJSON(const String& json, ProdigyBootstrapConfig& config, String *failure = nullptr)
{
  ...
  if (parser.parse(json.data(), json.size()).get(doc))
  {
    if (failure)
    {
      failure->assign("invalid bootstrap json");
    }
    return false;
  }
  ...
  if (doc.type() != simdjson::dom::element_type::OBJECT)
  {
    if (failure)
    {
      failure->assign("bootstrap config must be an object");
    }
    return false;
  }
  ...
  for (auto field : doc.get_object())
  {
    ...
    else
    {
      if (failure)
      {
        failure->assign("invalid bootstrap config field");
      }
      return false;
    }
  }
