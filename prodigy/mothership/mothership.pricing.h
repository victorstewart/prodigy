...
static inline void mothershipCountryDisplayFromKey(const String& key, String& display)
{
  ...
  if (key.equal("unitedstates"_ctv))
  {
    display.assign("United States"_ctv);
  }
  else if (key.equal("canada"_ctv))
  {
    display.assign("Canada"_ctv);
  }
  else if (key.equal("mexico"_ctv))
  {
    display.assign("Mexico"_ctv);
  }
  else if (key.equal("brazil"_ctv))
  {
    display.assign("Brazil"_ctv);
  }
  else if (key.equal("ireland"_ctv))
  {
    display.assign("Ireland"_ctv);
  }
  else if (key.equal("unitedkingdom"_ctv))
  {
    display.assign("United Kingdom"_ctv);
  }
  else if (key.equal("france"_ctv))
  {
    display.assign("France"_ctv);
  }
  else if (key.equal("germany"_ctv))
  {
    display.assign("Germany"_ctv);
  }
  else if (key.equal("switzerland"_ctv))
  {
    display.assign("Switzerland"_ctv);
  }
  else if (key.equal("sweden"_ctv))
  {
    display.assign("Sweden"_ctv);
  }
  else if (key.equal("italy"_ctv))
  {
    display.assign("Italy"_ctv);
  }
  else if (key.equal("spain"_ctv))
  {
    display.assign("Spain"_ctv);
  }
  else if (key.equal("poland"_ctv))
  {
    display.assign("Poland"_ctv);
  }
  else if (key.equal("finland"_ctv))
  {
    display.assign("Finland"_ctv);
  }
  else if (key.equal("netherlands"_ctv))
  {
    display.assign("Netherlands"_ctv);
  }
  else if (key.equal("belgium"_ctv))
  {
    display.assign("Belgium"_ctv);
  }
  else if (key.equal("southafrica"_ctv))
  {
    display.assign("South Africa"_ctv);
  }
  else if (key.equal("israel"_ctv))
  {
    display.assign("Israel"_ctv);
  }
  else if (key.equal("unitedarabemirates"_ctv))
  {
    display.assign("United Arab Emirates"_ctv);
  }
  else if (key.equal("india"_ctv))
  {
    display.assign("India"_ctv);
  }
  else if (key.equal("japan"_ctv))
  {
    display.assign("Japan"_ctv);
  }
  else if (key.equal("southkorea"_ctv))
  {
    display.assign("South Korea"_ctv);
  }
  else if (key.equal("singapore"_ctv))
  {
    display.assign("Singapore"_ctv);
  }
  else if (key.equal("hongkong"_ctv))
  {
    display.assign("Hong Kong"_ctv);
  }
  else if (key.equal("taiwan"_ctv))
  {
    display.assign("Taiwan"_ctv);
  }
  else if (key.equal("australia"_ctv))
  {
    display.assign("Australia"_ctv);
  }
  else if (key.equal("newzealand"_ctv))
  {
    display.assign("New Zealand"_ctv);
  }
  else if (key.equal("norway"_ctv))
  {
    display.assign("Norway"_ctv);
  }
  else
  {
    display.assign(key);
...
{
  constexpr static MothershipAwsRegionMapping mappings[] = {
...
{
  constexpr static MothershipCloudCountryMapping mappings[] = {
...
static inline bool mothershipParseGcpProviderScope(const String& scope, String& projectId, String& zone, String *failure = nullptr)
{
  ...
  if (scope.size() == 0)
  {
    if (failure)
    {
      failure->assign("gcp providerScope required"_ctv);
    }
    return false;
  }
  ...
  if (projectId.size() == 0 || zone.size() == 0)
  {
    if (failure)
    {
      failure->assign("gcp providerScope requires project and zone"_ctv);
    }
    return false;
  }
    ...
    String *failure = nullptr)
{
  ...
  if (provider == MothershipClusterProvider::aws)
  {
    if (awsScopeRegion(providerScope, resolvedScope) == false || mothershipResolveAwsRegionCountry(resolvedScope, countryKey) == false)
    {
      if (failure)
      {
        failure->assign("aws providerScope region is unsupported for country mapping"_ctv);
      }
      return false;
    }
  }
  else if (provider == MothershipClusterProvider::gcp)
  {
    ...
    if (mothershipResolveGcpRegionCountry(resolvedScope, countryKey) == false)
    {
      if (failure)
      {
        failure->assign("gcp providerScope region is unsupported for country mapping"_ctv);
      }
      return false;
    }
  }
  else if (provider == MothershipClusterProvider::azure)
  {
    ...
    if (mothershipResolveAzureLocationCountry(resolvedScope, countryKey) == false)
    {
      if (failure)
      {
        failure->assign("azure providerScope location is unsupported for country mapping"_ctv);
      }
      return false;
    }
  }
  else
  {
    if (failure)
    {
      failure->assign("unsupported provider for pricing"_ctv);
    }
    return false;
  }
    ...
    String *failure = nullptr)
{
  ...
  if (target.provider == MothershipClusterProvider::unknown)
  {
    if (failure)
    {
      failure->assign("provider required"_ctv);
    }
    return false;
  }
  ...
  if (target.hasProviderCredentialOverride)
  {
    ...
    else if (credential.provider != target.provider)
    {
      if (failure)
      {
        failure->assign("providerCredentialOverride provider does not match provider"_ctv);
      }
      return false;
    }
    ...
    else if (target.providerScope.equals(credential.scope) == false)
    {
      if (failure)
      {
        failure->assign("providerCredentialOverride scope does not match providerScope"_ctv);
      }
      return false;
    }
    ...
    if (credential.material.size() == 0)
    {
      if (failure)
      {
        failure->assign("providerCredentialOverride material required"_ctv);
      }
      return false;
    }
  ...
  if (target.providerCredentialName.size() == 0)
  {
    if (failure)
    {
      failure->assign("providerCredentialName or providerCredentialOverride required"_ctv);
    }
    return false;
  }
  ...
  if (credential.provider != target.provider)
  {
    if (failure)
    {
      failure->assign("providerCredentialName provider does not match provider"_ctv);
    }
    return false;
  }
  ...
  else if (credential.scope.size() > 0 && target.providerScope.equals(credential.scope) == false)
  {
    if (failure)
    {
      failure->assign("providerCredentialName scope does not match providerScope"_ctv);
    }
    return false;
  }
  ...
  if (target.providerScope.size() == 0)
  {
    if (failure)
    {
      failure->assign("providerScope required"_ctv);
    }
    return false;
  }
...
static inline bool mothershipParseUnsignedDecimalUSDToMicrousd(const simdjson::dom::element& value, uint64_t& microusd, String *failure = nullptr, const String& fieldName = "budgetHourlyUSD"_ctv)
{
  ...
  if (std::isfinite(hourly) == false || hourly < 0.0)
  {
    if (failure)
    {
      failure->snprintf<"{} must be >= 0"_ctv>(fieldName);
    }
    return false;
  }
...
static inline bool mothershipJsonMoneyMicrousd(simdjson::dom::element money, uint64_t& microusd)
{
  ...
  if (money["units"].get(signedUnits) == simdjson::SUCCESS)
  {
    ...
    microusd = uint64_t(signedUnits) * 1'000'000ull;
  ...
  if (money["units"].get(unitsText) == simdjson::SUCCESS)
  {
    ...
    microusd = uint64_t(parsedUnits) * 1'000'000ull;
    microusd += uint64_t(std::llround(double(nanos) / 1000.0));
    return true;
  }
...
static inline void mothershipNormalizeMachineSelections(Vector<MothershipMachineOfferSelection>& selections)
{
  ...
  for (const MothershipMachineOfferSelection& selection : selections)
  {
    ...
    if (normalized.empty() == false)
    {
      ...
      if (tail.kind == selection.kind && tail.providerMachineType == selection.providerMachineType && tail.storageMB == selection.storageMB)
      {
    ...
    String *failure = nullptr)
{
  ...
  for (const MothershipMachineOfferSelection& selection : selections)
  {
    ...
    if (mothershipLookupOfferByType(offers, selection.providerMachineType, selection.kind, offer) == false || offer == nullptr)
    {
      if (failure)
      {
        failure->snprintf<"offer '{}:{}' not found in surveyed provider scope"_ctv>(String(machineKindName(selection.kind)), selection.providerMachineType);
      }
      return false;
    }
    ...
    if (selectionStorageMicrousd == 0 && selection.storageMB > offer->nStorageMBDefault && offer->priceCompleteness != MothershipProviderOfferPriceCompleteness::computeStorageNetwork)
    {
      if (failure)
      {
        failure->snprintf<"offer '{}:{}' missing extra storage pricing"_ctv>(String(machineKindName(selection.kind)), selection.providerMachineType);
      }
      return false;
    }
  ...
  if (haveSelectedOffers == false)
  {
    if (failure)
    {
      failure->assign("no machine selections provided"_ctv);
    }
    return false;
  }
  ...
  if (allSelectedOffersComplete == false && (ingressMBPerHour > 0 || egressMBPerHour > 0))
  {
    if (failure)
    {
      failure->assign("selected offers are missing network pricing"_ctv);
    }
    return false;
  }
...
static inline uint64_t mothershipPlacementSlackScore(const Machine& machine, const ApplicationConfig& config)
{
  ...
  return (gpuSlack << 56) | ((cpuSlack > 0xffffffffull ? 0xffffffffull : cpuSlack) << 24) | ((memorySlack > 0xfffffull ? 0xfffffull : memorySlack) << 4) | (storageSlack > 0x0full ? 0x0full : storageSlack);
}
  ...
  for (uint32_t applicationIndex : order)
  {
    ...
    for (uint32_t instance = 0; instance < application.instances; ++instance)
    {
      ...
      for (uint32_t machineIndex = 0; machineIndex < machines.size(); ++machineIndex)
      {
        ...
        if (bestMachineIndex < 0 || (used && !bestUsed) || (used == bestUsed && slack < bestSlack))
        {
...
static inline uint64_t mothershipOfferRepresentativeExtraStorageMicrousdPerGBHour(const MothershipProviderMachineOffer& offer)
{
  ...
  return uint64_t(std::llround(
      double(nextTier->hourlyMicrousd - defaultTier->hourlyMicrousd) / double(nextTier->capacityGB - defaultTier->capacityGB)));
}
...
static inline bool mothershipOfferDominates(const MothershipProviderMachineOffer& lhs, const MothershipProviderMachineOffer& rhs)
{
  ...
  bool noWorse =
      lhs.hourlyMicrousd <= rhs.hourlyMicrousd && lhs.nLogicalCores >= rhs.nLogicalCores && lhs.nMemoryMB >= rhs.nMemoryMB && lhs.nStorageMBDefault >= rhs.nStorageMBDefault && lhs.gpuCount >= rhs.gpuCount && lhs.gpuMemoryMBPerDevice >= rhs.gpuMemoryMBPerDevice && lhs.nicSpeedMbps >= rhs.nicSpeedMbps && lhsStorageRate <= rhsStorageRate && lhs.ingressMicrousdPerGB <= rhs.ingressMicrousdPerGB && lhs.egressMicrousdPerGB <= rhs.egressMicrousdPerGB && lhs.providesHostPublic4 >= rhs.providesHostPublic4 && lhs.providesHostPublic6 >= rhs.providesHostPublic6;
  ...
  return lhs.hourlyMicrousd < rhs.hourlyMicrousd || lhs.nLogicalCores > rhs.nLogicalCores || lhs.nMemoryMB > rhs.nMemoryMB || lhs.nStorageMBDefault > rhs.nStorageMBDefault || lhs.gpuCount > rhs.gpuCount || lhs.gpuMemoryMBPerDevice > rhs.gpuMemoryMBPerDevice || lhs.nicSpeedMbps > rhs.nicSpeedMbps || lhsStorageRate < rhsStorageRate || lhs.ingressMicrousdPerGB < rhs.ingressMicrousdPerGB || lhs.egressMicrousdPerGB < rhs.egressMicrousdPerGB || lhs.providesHostPublic4 != rhs.providesHostPublic4 || lhs.providesHostPublic6 != rhs.providesHostPublic6;
}
    ...
    String& failure)
{
  ...
  if (mothershipResolveAwsRegionCountry(region, expectedCountryKey) == false || mothershipCountryMatchesRequested(request.country, expectedCountryKey.c_str(), &failure) == false)
  {
    if (failure.size() == 0)
    {
  ...
  while (true)
  {
    ...
    for (auto encodedEntry : priceList.get_array())
    {
      ...
      String memoryText = {};
      uint32_t vcpu = 0;
      if (mothershipJsonGetString(attributes, "instanceType", instanceType) == false || mothershipJsonGetString(attributes, "memory", memoryText) == false || mothershipJsonGetUInt32StringOrNumber(attributes, "vcpu", vcpu) == false)
      {
        continue;
      }
    ...
    String& failure)
{
  ...
  String expectedCountryKey = {};
  if (mothershipResolveGcpRegionCountry(region, expectedCountryKey) == false || mothershipCountryMatchesRequested(request.country, expectedCountryKey.c_str(), &failure) == false)
  {
    if (failure.size() == 0)
    {
  ...
  while (true)
  {
    ...
    if (doc["skus"].is_array())
    {
      for (auto sku : doc["skus"].get_array())
      {
        ...
        if ((mothershipTextContains(description, "BALANCED PD CAPACITY") || mothershipTextContains(description, "BALANCED PERSISTENT DISK CAPACITY") || mothershipTextContains(description, "PD BALANCED CAPACITY")) && variableRateMicrousd > 0 && (storageMicrousdPerGBHour == 0 || variableRateMicrousd < storageMicrousdPerGBHour))
        {
          storageMicrousdPerGBHour = variableRateMicrousd;
        }

        if (mothershipTextContains(description, "NETWORK INTERNET EGRESS") && mothershipTextContains(description, "WORLDWIDE") && mothershipTextContains(description, "CHINA") == false && mothershipTextContains(description, "AUSTRALIA") == false && mothershipTextContains(description, "INTERCONNECT") == false && mothershipTextContains(description, "VPN") == false && variableRateMicrousd > egressMicrousdPerGB)
        {
          egressMicrousdPerGB = variableRateMicrousd;
        }

        constexpr static const char *families[] = {
    ...
    String& failure)
{
  ...
  String expectedCountryKey = {};
  if (mothershipResolveAzureLocationCountry(location, expectedCountryKey) == false || mothershipCountryMatchesRequested(request.country, expectedCountryKey.c_str(), &failure) == false)
  {
    if (failure.size() == 0)
    {
  ...
  while (nextLink.size() > 0)
  {
    ...
    if (doc["value"].is_array())
    {
      for (auto sku : doc["value"].get_array())
      {
        ...
        uint32_t vcpus = 0;
        uint32_t memoryGB = 0;
        if (mothershipAzureCapabilityUInt32(sku, "vCPUs", vcpus) == false || mothershipAzureCapabilityUInt32(sku, "MemoryGB", memoryGB) == false)
        {
          continue;
        }
        ...
        offer.providesHostPublic4 = providesHostPublic4;
        offer.providesHostPublic6 = providesHostPublic6;
        offer.freeTierEligible = request.billingModel == ProviderMachineBillingModel::hourly && (name.equal("Standard_B1s"_ctv) || name.equal("Standard_B2ts_v2"_ctv) || name.equal("Standard_B2pts_v2"_ctv) || name.equal("Standard_B2ats_v2"_ctv));
  ...
  while (nextPage.size() > 0)
  {
    ...
    if (doc["Items"].is_array())
    {
      for (auto item : doc["Items"].get_array())
      {
        ...
        bool isSpotEntry = mothershipTextContains(productName, "SPOT") || mothershipTextContains(productName, "LOW PRIORITY") || mothershipTextContains(meterName, "SPOT") || mothershipTextContains(meterName, "LOW PRIORITY") || mothershipTextContains(skuName, "SPOT") || mothershipTextContains(skuName, "LOW PRIORITY");
    ...
    MothershipClusterRecommendation& recommendation)
{
  ...
  auto considerSelections = [&](Vector<MothershipMachineOfferSelection> selections) -> void {
    ...
    else if (hourlyMicrousd == bestHourlyMicrousd && totalMachines == bestTotalMachines && mothershipMachineSelectionsLess(realizedSelections, bestSelections))
    {
      better = true;
    }
