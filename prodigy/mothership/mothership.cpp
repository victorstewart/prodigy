...
static bool mothershipParseEndpointHostPort(const String& endpoint, String& host, uint16_t& port, String *failure = nullptr)
{
  host.clear();
  port = 0;
...
static bool parseMothershipConnectivityJSON(simdjson::dom::element value, MothershipConnectivity& connectivity, String& providerContainerBlobPath, const char *context)
{
  ...
  if (sawKind == false)
  {
    basics_log("%s.kind required\n", context);
    return false;
  }

  if (parsed.kind == MothershipConnectivityKind::ssh && sawTunnelField)
  {
    basics_log("%s kind=ssh must not include tunnelProvider fields\n", context);
    return false;
  }
...
static void printManagedCluster(const MothershipProdigyCluster& cluster)
{
  ...
  basics_log("    mothershipConnectivity kind=%s\n", mothershipConnectivityKindName(cluster.mothershipConnectivity.kind));
...
constexpr static uint32_t radarMinimumLogicalCores = 3;
...
static bool mothershipReadProcEnvironmentValue(uint32_t pid, const char *name, String& value)
{
  ...
  while (offset < environment.size())
  {
    ...
    if (end > offset + nameLength && std::memcmp(environment.data() + offset, name, nameLength) == 0 && environment[offset + nameLength] == '=')
    {
...
static bool mothershipReadProcCommandLineValue(uint32_t pid, const char *name, String& value)
{
  ...
  while (offset < commandLine.size())
  {
    ...
    if (end > offset + nameLength && std::memcmp(commandLine.data() + offset, name, nameLength) == 0 && commandLine[offset + nameLength] == '=')
    {
...
static bool resolveLiveLocalProdigyControlSocketPath(String& controlSocketPath, String *failure = nullptr)
{
  ...
  if (proc == nullptr)
  {
    if (failure)
    {
      failure->snprintf<"failed to open /proc: {}"_ctv>(String(std::strerror(errno)));
    }
    return false;
  }
  ...
  while (dirent *entry = ::readdir(proc))
  {
    ...
    String socketPath = {};
    if (mothershipReadProcBootJSONControlSocketPath(pid, socketPath) == false && mothershipReadProcEnvironmentValue(pid, "PRODIGY_MOTHERSHIP_SOCKET", socketPath) == false)
    {
      continue;
    }
  ...
  if (failure)
  {
    failure->assign("no live local prodigy process found"_ctv);
  }
  return false;
}
...
static bool loadLocalProdigyControlSocketPath(String& controlSocketPath, String *failure = nullptr)
{
  if (const char *explicitSocketPath = getenv("PRODIGY_MOTHERSHIP_SOCKET"))
  {
    if (explicitSocketPath[0] == '\0')
    {
      if (failure)
      {
        failure->assign("PRODIGY_MOTHERSHIP_SOCKET is empty"_ctv);
      }
      return false;
    }
    ...
    if (failure)
    {
      failure->snprintf<"PRODIGY_MOTHERSHIP_SOCKET is not a live unix socket: {}"_ctv>(controlSocketPath);
    }
    return false;
  }
  ...
  if (bootState.bootstrapConfig.controlSocketPath.size() == 0)
  {
    if (failure)
    {
      failure->assign("local prodigy control socket path missing from boot state"_ctv);
    }
    return false;
  }
...
static bool resolveLocalProdigyExecutablePath(String& localProdigyPath, String *failure = nullptr)
{
  ...
  if (failure)
  {
    failure->assign("failed to locate local prodigy executable"_ctv);
  }
  return false;
}
...
static bool bootstrapLocalProdigy(const ProdigyPersistentBootState& bootState, String *failure = nullptr)
{
  ...
  if (prodigyMachineCpuArchitectureSupportedTarget(sourceArchitecture) == false)
  {
    if (failure)
    {
      failure->assign("local build architecture unsupported for installed prodigy bundle selection"_ctv);
    }
    return false;
  }
  ...
  if (Filesystem::openWriteAtClose(-1, "/etc/systemd/system/prodigy.service.tmp"_ctv, systemdUnit) < 0)
  {
    if (failure)
    {
      failure->assign("failed to write local prodigy systemd unit"_ctv);
    }
    return false;
  }
  ...
  if (Filesystem::renameFile("/etc/systemd/system/prodigy.service.tmp"_ctv, "/etc/systemd/system/prodigy.service"_ctv) != 0)
  {
    if (failure)
    {
      failure->assign("failed to install local prodigy systemd unit"_ctv);
    }
    return false;
  }
...
private:
  ...
  constexpr static int defaultSshTransportIOTimeoutMs = 120'000;
  ...
  const char *transportModeName(void) const
  {
    switch (transportMode)
    {
      ...
      case TransportMode::remoteSshUnix:
        return "remoteSshUnix";
      case TransportMode::tunnelGateway:
  ...
  bool connectTunnelGateway(void)
  {
    ...
    std::unique_ptr<int, void (*)(int *)> closeFD(&fd, [](int *value) {
      if (*value >= 0)
      {
        ::close(*value);
      }
...
public:
  ...
  bool configureCluster(const MothershipProdigyCluster& cluster, String *failure = nullptr)
  {
    ...
    if (controlPaths.size() == 0)
    {
      if (failure)
      {
        failure->assign("cluster has no unixSocket controls"_ctv);
      }
      return false;
    }
  ...
  bool configureClusterBootstrapControl(const MothershipProdigyCluster& cluster, String *failure = nullptr)
  {
    MothershipProdigyCluster bootstrapCluster = cluster;
    bootstrapCluster.mothershipConnectivity = {};
    return configureCluster(bootstrapCluster, failure);
  }
  ...
  int connect(void)
  {
    ...
    switch (transportMode)
    {
      ...
      case TransportMode::tunnelGateway:
        {
          return connectTunnelGateway() ? 0 : -1;
        }
      case TransportMode::none:
        {
  ...
  bool ensureConnected(void)
  {
    ...
    else if (transportMode == TransportMode::tunnelGateway && (tunnelGatewayTLS == nullptr || transportFD < 0))
    {
      return connect() == 0;
    }
...
private:
      ...
      const String *debugSystemStoreRoot = nullptr)
  {
    ...
    if (blob.size() == 0)
    {
      if (failure)
      {
        failure->assign("tunnelProvider.providerContainerBlobPath unreadable or empty"_ctv);
      }
      return false;
    }
    ...
    if (stored == false)
    {
      if (failure)
      {
        failure->snprintf<"tunnelProvider artifact store failed: {}"_ctv>(storeFailure);
      }
      return false;
    }
  ...
  static void ensureClusterUUIDForCreate(MothershipProdigyCluster& cluster)
  {
  ...
  static bool resolveProdigyBundleTargetArchitecture(const char *arg, MachineCpuArchitecture& architecture, String *failure = nullptr)
  {
    ...
    if (arg == nullptr || arg[0] == '\0')
    {
      if (failure)
      {
        failure->assign("control target required"_ctv);
      }
      return false;
    }
    ...
    if (std::strcmp(arg, "local") == 0)
    {
      ...
      if (prodigyMachineCpuArchitectureSupportedTarget(architecture) == false)
      {
        if (failure)
        {
          failure->assign("local build architecture unsupported for prodigy bundle selection"_ctv);
        }
        return false;
      }
    ...
    if (prodigyMachineCpuArchitectureSupportedTarget(cluster.architecture) == false)
    {
      if (failure)
      {
        failure->snprintf<"cluster architecture must be x86_64, aarch64, or riscv64 for bundle selection: {}"_ctv>(clusterIdentity);
      }
      return false;
    }
  ...
  {
    if (cluster.deploymentMode == MothershipClusterDeploymentMode::local || cluster.deploymentMode == MothershipClusterDeploymentMode::test)
    {
      failure.clear();
      return true;
    }
  ...
  static bool resolveClusterUnixControlPath(const MothershipProdigyCluster& cluster, String& controlSocketPath, String *failure = nullptr)
  {
    ...
    if (failure)
    {
      failure->assign("cluster has no unixSocket control path"_ctv);
    }
    return false;
  }
  ...
  bool buildLocalClusterMachineProbe(const MothershipProdigyCluster& cluster, ClusterMachine& localMachine, String *failure = nullptr)
  {
    ...
    if (localCandidates.empty())
    {
      if (failure)
      {
        failure->assign("failed to resolve local peer addresses"_ctv);
      }
      return false;
    }
      ...
      String& failure)
  {
    ...
    if (desiredCluster.deploymentMode == MothershipClusterDeploymentMode::local && (desiredCluster.includeLocalMachine || mothershipClusterIncludesLocalMachine(controlCluster)))
    {
    ...
    if (mothershipBuildDesiredClusterReconcileRequest(
            ...
            changed,
            &failure,
            ((desiredCluster.deploymentMode == MothershipClusterDeploymentMode::local && (desiredCluster.includeLocalMachine || mothershipClusterIncludesLocalMachine(controlCluster)))
                 ? &localMachine
                 : nullptr),
  ...
  private:
    ...
    bool waitForClusterRequest(const MothershipProdigyCluster& cluster, auto&& action, String *failure = nullptr)
    {
      ...
      for (;;)
      {
        ...
        bool configured = owner->socket.configureClusterBootstrapControl(cluster, &lastFailure);
        if (configured)
        {
    ...
    bool ensureCloudMachinesTaggedLocally(const MothershipProdigyCluster& cluster, const Vector<ClusterMachine>& machines, String *failure = nullptr)
    {
      ...
      if (provider == nullptr)
      {
        if (failure)
        {
          failure->assign("failed to construct runtime provider for cloud machine tagging"_ctv);
        }
        return false;
      }
  ...
  public:
    ...
    bool prepareProviderBootstrapArtifacts(const MothershipProdigyCluster& cluster, ProdigyTimingAttribution *timingAttribution = nullptr, String *failure = nullptr) override
    {
      ...
      if (cluster.deploymentMode != MothershipClusterDeploymentMode::remote || (cluster.provider != MothershipClusterProvider::gcp && cluster.provider != MothershipClusterProvider::azure))
      {
        if (failure)
        {
          failure->clear();
    ...
    bool createSeedMachine(const MothershipProdigyCluster& cluster, const CreateMachinesInstruction& instruction, ClusterMachine& seedMachine, ProdigyTimingAttribution *timingAttribution = nullptr, String *failure = nullptr) override
    {
      ...
      if (provider == nullptr)
      {
        if (failure)
        {
          failure->assign("failed to construct runtime provider for cluster seed provisioning"_ctv);
        }
        return false;
      }
    ...
    bool destroyCreatedSeedMachine(const MothershipProdigyCluster& cluster, const ClusterMachine& seedMachine, String *failure = nullptr) override
    {
      ...
      if (provider == nullptr)
      {
        if (failure)
        {
          failure->assign("failed to construct runtime provider for cluster seed destroy"_ctv);
        }
        return false;
      }
      provider->configureRuntimeEnvironment(runtimeEnvironment);
    ...
    bool configureSeedCluster(const MothershipProdigyCluster& cluster, const BrainConfig& config, String *failure = nullptr) override
    {
      ...
      return waitForClusterRequest(cluster, [&](String& requestFailure) -> bool {
        ...
        auto recvMothershipResponse = [&](MothershipTopic topic, const auto& timeoutFailure, const auto& rejectedFailure) -> bool {
          Message *responseMessage = owner->socket.recvExpectedTopic(topic);
          if (responseMessage == nullptr)
          {
            if (owner->socket.ioFailureDetail().size() > 0)
            {
            ...
            {
              requestFailure.assign(timeoutFailure);
            }
            return false;
          }
          ...
          String serializedResponse = {};
          Message::extractToStringView(responseArgs, serializedResponse);
          MothershipResponse decodedResponse = {};
          if (BitseryEngine::deserializeSafe(serializedResponse, decodedResponse) == false || decodedResponse.success == false)
          {
            if (decodedResponse.failure.size() > 0)
            {
              requestFailure = decodedResponse.failure;
            }
            else
            {
              requestFailure.assign(rejectedFailure);
            }
            return false;
          }
        ...
        if (cluster.mothershipConnectivity.kind == MothershipConnectivityKind::tunnelProvider)
        {
          ...
          if (ContainerStore::systemLoadVerified(providerSpec.artifactSha256, providerSpec.artifactBytes, artifactBlob, &artifactFailure) == false)
          {
            ...
            {
              requestFailure.snprintf<"tunnelProvider artifact load failed: {}"_ctv>(artifactFailure);
            }
            else
            {
              requestFailure.assign("tunnelProvider artifact load failed"_ctv);
            }
            return false;
          }
    ...
    bool upsertMachineSchemas(const MothershipProdigyCluster& cluster, const Vector<ProdigyManagedMachineSchemaPatch>& patches, ClusterTopology& topology, ProdigyTimingAttribution *timingAttribution = nullptr, String *failure = nullptr) override
    {
      ...
      if (response.hasTopology == false)
      {
        if (failure)
        {
          failure->assign("cluster upsertMachineSchemas response missing topology"_ctv);
        }
        return false;
      }
  ...
  void runRecoverRetainedFleet(int argc, char *argv[])
  {
    ...
    const bool repair=argc==3 && (std::strcmp(argv[1], "recover")==0 || std::strcmp(argv[1], "prepare")==0 || std::strcmp(argv[1], "compact-contained")==0 || std::strcmp(argv[1], "resume-compacted")==0);
  ...
  void runUpsertMachineSchemas(int argc, char *argv[])
  {
    ...
    if (validateClusterProviderCredentialReference(desiredCluster, failure, &referencedCredential) == false || inferClusterMachineSchemaCpuCapabilities(desiredCluster, &referencedCredential, failure) == false)
    {
      basics_log("upsertMachineSchemas success=0 name=%s failure=%s\n", controlCluster.name.c_str(), failure.c_str());
      exit(EXIT_FAILURE);
    }
  ...
  void runRecommendClusterForApplications(int argc, char *argv[])
  {
    ...
    for (auto field : doc.get_object())
    {
      ...
      {
        uint64_t value = 0;
        if ((field.value.type() != simdjson::dom::element_type::INT64 && field.value.type() != simdjson::dom::element_type::UINT64) || field.value.get(value) != simdjson::SUCCESS || value == 0 || value > UINT32_MAX)
        {
    ...
    for (const MothershipProviderScopeTarget& target : activeTargets)
    {
      ...
      else if (candidate.hourlyMicrousd == best.hourlyMicrousd && candidate.totalMachines == best.totalMachines && candidate.target.provider != best.target.provider)
      {
  ...
  void runCreateCluster(int argc, char *argv[])
  {
    ...
    for (auto field : doc.get_object())
    {
      ...
      else if (key.equal("architecture"_ctv))
      {
        ...
        if (parseMachineCpuArchitecture(architecture, request.architecture) == false || prodigyMachineCpuArchitectureSupportedTarget(request.architecture) == false)
        {
      ...
      else if (key.equal("test"_ctv))
      {
        ...
        for (auto testField : field.value.get_object())
        {
          ...
          {
            int64_t value = 0;
            if (testField.value.type() != simdjson::dom::element_type::INT64 || testField.value.get(value) != simdjson::SUCCESS || value < 0 || value > INT32_MAX)
            {
          ...
          {
            uint64_t value = 0;
            if ((testField.value.type() != simdjson::dom::element_type::INT64 && testField.value.type() != simdjson::dom::element_type::UINT64) || testField.value.get(value) != simdjson::SUCCESS || value > UINT32_MAX)
            {
      ...
      {
        uint64_t value = 0;
        if ((field.value.type() != simdjson::dom::element_type::INT64 && field.value.type() != simdjson::dom::element_type::UINT64) || field.value.get(value) != simdjson::SUCCESS || value == 0 || value > 86'400)
        {
      ...
      {
        if (field.value.type() != simdjson::dom::element_type::BOOL || field.value.get(request.osUpdatesEnabled) != simdjson::SUCCESS)
        {
          basics_log("createCluster.osUpdatesEnabled requires bool\n");
          exit(EXIT_FAILURE);
        }
      }
      else if (key.equal("targetOSID"_ctv) || key.equal("targetOSVersionID"_ctv) || key.equal("osUpdateCommand"_ctv) || key.equal("includeVMsInOSUpdates"_ctv))
      {
  ...
  void runUpsertTlsVaultFactory(int argc, char *argv[])
  {
    ...
    for (auto field : doc.get_object())
    {
      ...
      else if (key.equal("mode"_ctv))
      {
        ...
        String mode;
        mode.setInvariant(field.value.get_c_str());
        if (mode.equal("generate"_ctv))
        {
          request.mode = 0;
        }
        else if (mode.equal("import"_ctv))
        {
          request.mode = 1;
        }
        else
      ...
      else if (key.equal("scheme"_ctv))
      {
        ...
        if (scheme.equal("p256"_ctv))
        {
          request.scheme = 0;
        }
        else if (scheme.equal("ed25519"_ctv))
        {
          request.scheme = 1;
        }
        else
        ...
        for (auto item : field.value.get_array())
        {
          ...
          for (auto credField : item.get_object())
          {
            ...
            else if (credKey.equal("generation"_ctv))
            {
              ...
              if (value > 0)
              {
                credential.generation = uint64_t(value);
              }
            }
...
public:
  ...
  bool unitTestConnectConfiguredSocket(String& failure)
  {
    bool ok = (socket.connect() == 0);
    failure = socket.connectFailureDetail();
    return ok;
  }
