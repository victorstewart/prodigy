...
{
  return failure.size() >= mothershipNonRetryableFailurePrefix.size() && std::memcmp(failure.data(), mothershipNonRetryableFailurePrefix.data(), size_t(mothershipNonRetryableFailurePrefix.size())) == 0;
}
