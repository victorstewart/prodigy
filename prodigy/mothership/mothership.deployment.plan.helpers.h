    ...
    String *failure = nullptr)
{
  ...
  for (auto subfield : value.get_object())
  {
    ...
    {
      int64_t parsed = 0;
      if (subfield.value.type() != simdjson::dom::element_type::INT64 || subfield.value.get(parsed) != simdjson::SUCCESS || parsed <= 0 || parsed > 825)
      {
        if (failure)
        {
    ...
    String *failure = nullptr)
{
  ...
  if (parseMachineCpuArchitecture(text, config.architecture) == false || prodigyMachineCpuArchitectureSupportedTarget(config.architecture) == false)
  {
    if (failure)
    {
    ...
    String *failure = nullptr)
{
  ...
  if (sharedCPUs == false)
  {
    if (value.type() != simdjson::dom::element_type::INT64 && value.type() != simdjson::dom::element_type::UINT64)
    {
      if (failure)
      {
  ...
  if (value.type() != simdjson::dom::element_type::DOUBLE && value.type() != simdjson::dom::element_type::INT64 && value.type() != simdjson::dom::element_type::UINT64)
  {
    if (failure)
    {
    ...
    const String& fieldName = "sharedCpuOvercommit"_ctv)
{
  ...
  if (std::isfinite(ratio) == false || ratio < 1.0 || ratio > 2.0)
  {
    if (failure)
    {
  ...
  if (permille < prodigySharedCPUOvercommitMinPermille || permille > prodigySharedCPUOvercommitMaxPermille)
  {
    if (failure)
    {
      failure->snprintf<"{} must be in 1.0..2.0"_ctv>(fieldName);
