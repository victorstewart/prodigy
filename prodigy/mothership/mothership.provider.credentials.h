...
private:
  ...
  constexpr static auto credentialsColumnFamily = "provider_credentials"_ctv;
  ...
  static bool buildAzureBootstrapAccessTokenRefreshCommand(const MothershipProviderCredential& credential, String& command, String *failure = nullptr)
  {
    ...
    if (failure)
    {
      failure->assign("provider credential mode invalid");
    }
    return false;
  }
      ...
      case MothershipProviderCredentialMode::azureCli:
        {
          if (failure)
          {
            failure->assign("azureCli provider credential mode is invalid for GCP refresh");
          }
          return false;
        }
      case MothershipProviderCredentialMode::awsCli:
        {
          if (failure)
          {
            failure->assign("awsCli provider credential mode is invalid for GCP refresh");
          }
          return false;
        }
      case MothershipProviderCredentialMode::awsImds:
        {
          if (failure)
          {
            failure->assign("awsImds provider credential mode is invalid for GCP refresh");
          }
          return false;
        }
    }
  ...
  static bool normalizeCredentialForStorage(MothershipProviderCredential& credential, bool creating, String *failure)
  {
    if (stringHasContent(credential.name) == false)
    {
      if (failure)
      {
        failure->assign("provider credential name required");
      }
      return false;
    }
    ...
    if (credential.provider == MothershipClusterProvider::unknown)
    {
      if (failure)
      {
        failure->assign("provider credential provider required");
      }
      return false;
    }

    if ((credential.mode == MothershipProviderCredentialMode::gcloud || credential.mode == MothershipProviderCredentialMode::gcloudImpersonation || credential.mode == MothershipProviderCredentialMode::externalAccountFile) && credential.provider != MothershipClusterProvider::gcp)
    {
      if (failure)
      {
        failure->assign("non-static provider credential modes currently require provider=gcp");
      }
      return false;
    }

    if (credential.mode == MothershipProviderCredentialMode::azureCli && credential.provider != MothershipClusterProvider::azure)
    {
      if (failure)
      {
        failure->assign("azureCli provider credential mode requires provider=azure");
      }
      return false;
    }

    if (credential.mode == MothershipProviderCredentialMode::awsCli && credential.provider != MothershipClusterProvider::aws)
    {
      if (failure)
      {
        failure->assign("awsCli provider credential mode requires provider=aws");
      }
      return false;
    }

    if (credential.mode == MothershipProviderCredentialMode::awsImds && credential.provider != MothershipClusterProvider::aws)
    {
      if (failure)
      {
        failure->assign("awsImds provider credential mode requires provider=aws");
      }
      return false;
    }
    ...
    if (credential.mode == MothershipProviderCredentialMode::staticMaterial)
    {
      if (stringHasContent(credential.material) == false)
      {
        if (failure)
        {
          failure->assign("provider credential material required");
        }
        return false;
      }
    }
    ...
    if (credential.mode == MothershipProviderCredentialMode::gcloudImpersonation && stringHasContent(credential.impersonateServiceAccount) == false)
    {
      if (failure)
      {
        failure->assign("provider credential impersonateServiceAccount required");
      }
      return false;
    }
    ...
    if (credential.mode == MothershipProviderCredentialMode::externalAccountFile)
    {
      if (stringHasContent(credential.credentialPath) == false)
      {
        if (failure)
        {
          failure->assign("provider credential credentialPath required");
        }
        return false;
      }
      if (credential.credentialPath[0] != '/')
      {
        if (failure)
        {
          failure->assign("provider credential credentialPath must be absolute");
        }
        return false;
      }
    }
    ...
    if (credential.allowPropagateToProdigy && credential.mode != MothershipProviderCredentialMode::staticMaterial)
    {
      if (failure)
      {
        failure->assign("only staticMaterial provider credentials may propagate to Prodigy");
      }
      return false;
    }
...
public:
  ...
  static bool applyCredentialToRuntimeEnvironment(const MothershipProviderCredential& credential, ProdigyRuntimeEnvironmentConfig& runtimeEnvironment, String *failure = nullptr)
  {
    ...
    if (credential.provider == MothershipClusterProvider::gcp && credential.mode != MothershipProviderCredentialMode::staticMaterial)
    {
    ...
    else if (credential.provider == MothershipClusterProvider::azure && credential.mode == MothershipProviderCredentialMode::azureCli)
    {
    ...
    else if (credential.provider == MothershipClusterProvider::aws && credential.mode == MothershipProviderCredentialMode::awsCli)
    {
  ...
  bool createCredential(const MothershipProviderCredential& credential, MothershipProviderCredential *storedCredential = nullptr, String *failure = nullptr)
  {
    ...
    if (exists)
    {
      if (failure)
      {
        failure->assign("provider credential already exists");
      }
      return false;
    }
  ...
  bool getCredential(const String& name, MothershipProviderCredential& credential, String *failure = nullptr)
  {
    ...
    if (deserializeCredentialValue(reinterpret_cast<const uint8_t *>(serialized.data()), serialized.size(), credential) == false)
    {
      if (failure)
      {
        failure->assign("provider credential decode failed");
      }
      return false;
    }
