...
static inline bool resolveMothershipClusterInlineProviderCredentialOverride(MothershipProdigyCluster& cluster, MothershipProviderCredential& credential, String *failure = nullptr)
{
  if (cluster.deploymentMode == MothershipClusterDeploymentMode::local || cluster.deploymentMode == MothershipClusterDeploymentMode::test)
  {
    if (failure)
    {
      failure->assign("providerless clusters must not include providerCredentialOverride");
    }
    return false;
  }
  ...
  else if (cluster.provider != credential.provider)
  {
    if (failure)
    {
      failure->assign("providerCredentialOverride provider does not match cluster provider");
    }
    return false;
  }
  ...
  else if (cluster.providerScope.equals(credential.scope) == false)
  {
    if (failure)
    {
      failure->assign("providerCredentialOverride scope does not match providerScope");
    }
    return false;
  }
  ...
  else if (credential.name.equals(cluster.providerCredentialName) == false)
  {
    if (failure)
    {
      failure->assign("providerCredentialOverride name does not match providerCredentialName");
    }
    return false;
  }
