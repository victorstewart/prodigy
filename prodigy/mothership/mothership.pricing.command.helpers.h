...
{
  return provider == MothershipClusterProvider::aws || provider == MothershipClusterProvider::gcp || provider == MothershipClusterProvider::azure;
}
    ...
    String *failure = nullptr)
{
  ...
  if (value.type() != simdjson::dom::element_type::ARRAY)
  {
    if (failure)
    {
      failure->assign("providers requires an array"_ctv);
    }
    return false;
  }
  ...
  for (auto item : value.get_array())
  {
    if (item.type() != simdjson::dom::element_type::STRING)
    {
      if (failure)
      {
        failure->assign("providers requires string members"_ctv);
      }
      return false;
    }
    ...
    if (parseMothershipClusterProvider(providerName, provider) == false || provider == MothershipClusterProvider::unknown)
    {
      if (failure)
      {
        failure->snprintf<"providers '{}' is invalid"_ctv>(providerName);
      }
      return false;
    }
  ...
  if (allProviders && providers.empty() == false)
  {
    if (failure)
    {
      failure->assign("providers may specify 'all' or explicit providers, not both"_ctv);
    }
    return false;
  }
  ...
  if (allProviders == false && providers.empty())
  {
    if (failure)
    {
      failure->assign("providers requires at least one provider or 'all'"_ctv);
    }
    return false;
  }
    ...
    String *failure = nullptr)
{
  ...
  if (value.type() != simdjson::dom::element_type::ARRAY)
  {
    if (failure)
    {
      failure->assign("machineKinds requires an array"_ctv);
    }
    return false;
  }
  ...
  for (auto item : value.get_array())
  {
    if (item.type() != simdjson::dom::element_type::STRING)
    {
      if (failure)
      {
        failure->assign("machineKinds requires string members"_ctv);
      }
      return false;
    }
    ...
    else
    {
      if (failure)
      {
        failure->snprintf<"machineKinds '{}' is invalid"_ctv>(kindText);
      }
      return false;
    }
  ...
  if (machineKindsMask == 0)
  {
    if (failure)
    {
      failure->assign("machineKinds requires at least one kind"_ctv);
    }
    return false;
  }
    ...
    String *failure = nullptr)
{
  ...
  for (MothershipClusterProvider provider : providers)
  {
    ...
    for (const MothershipProviderCredential& credential : credentials)
    {
      ...
      if (requestedCredentialNames.empty() == false && mothershipPricingStringVectorContains(requestedCredentialNames, credential.name) == false)
      {
        continue;
      }
    ...
    String *failure = nullptr)
{
  ...
  if (value.type() != simdjson::dom::element_type::OBJECT)
  {
    if (failure)
    {
      failure->assign("applications[].config requires an object"_ctv);
    }
    return false;
  }
  ...
  for (auto field : value.get_object())
  {
    ...
    {
      if (field.value.type() != simdjson::dom::element_type::DOUBLE && field.value.type() != simdjson::dom::element_type::INT64 && field.value.type() != simdjson::dom::element_type::UINT64)
      {
        if (failure)
        {
          failure->assign("applications[].config.nLogicalCores requires a number"_ctv);
        }
        return false;
      }
    ...
    else
    {
      ...
      else if (key.equal("machineResourceCriteria"_ctv))
      {
        if (failure)
        {
          failure->assign("applications[].config.machineResourceCriteria removed; place minGPUs, gpuMemoryGB, and nicSpeedGbps directly on applications[].config"_ctv);
        }
        return false;
      }
      else if (key.equal("minGPUs"_ctv))
      {
        if (mothershipParseJSONUInt32(field.value, config.minGPUs) == false)
        {
          if (failure)
          {
            failure->assign("applications[].config.minGPUs invalid"_ctv);
          }
          return false;
        }
      }
      else if (key.equal("gpuMemoryGB"_ctv))
      {
        if (mothershipParseJSONUInt32(field.value, config.gpuMemoryGB) == false)
        {
          if (failure)
          {
            failure->assign("applications[].config.gpuMemoryGB invalid"_ctv);
          }
          return false;
        }
      }
      else if (key.equal("nicSpeedGbps"_ctv))
      {
        if (mothershipParseJSONUInt32(field.value, config.nicSpeedGbps) == false)
        {
          if (failure)
          {
            failure->assign("applications[].config.nicSpeedGbps invalid"_ctv);
          }
          return false;
        }
      }
      else if (key.equal("minInternetDownloadMbps"_ctv) || key.equal("minInternetUploadMbps"_ctv) || key.equal("maxInternetLatencyMs"_ctv))
      {
        if (failure)
        {
          failure->snprintf<"applications[].config.{} is not supported for planning"_ctv>(key);
        }
        return false;
      }
      else
      {
        if (failure)
        {
          failure->snprintf<"applications[].config.{} is not recognized"_ctv>(key);
        }
        return false;
      }
    }
  ...
  if (config.nLogicalCores == 0)
  {
    if (failure)
    {
      failure->assign("applications[].config.nLogicalCores required"_ctv);
    }
    return false;
  }
  ...
  if (config.memoryMB == 0)
  {
    if (failure)
    {
      failure->assign("applications[].config.memoryMB or memoryGB required"_ctv);
    }
    return false;
  }
  ...
  if (config.gpuMemoryGB > 0 && config.minGPUs == 0)
  {
    if (failure)
    {
      failure->assign("applications[].config.gpuMemoryGB requires minGPUs > 0"_ctv);
    }
    return false;
  }
  ...
  if (config.gpuMemoryGB > (UINT32_MAX / 1024u))
  {
    if (failure)
    {
      failure->assign("applications[].config.gpuMemoryGB exceeds supported range"_ctv);
    }
    return false;
  }
    ...
    String *failure = nullptr)
{
  ...
  if (value.type() != simdjson::dom::element_type::ARRAY)
  {
    if (failure)
    {
      failure->assign("applications requires an array"_ctv);
    }
    return false;
  }
  ...
  for (auto item : value.get_array())
  {
    if (item.type() != simdjson::dom::element_type::OBJECT)
    {
      if (failure)
      {
        failure->assign("applications requires object members"_ctv);
      }
      return false;
    }
    ...
    for (auto field : item.get_object())
    {
      ...
      if (key.equal("name"_ctv))
      {
        if (field.value.type() != simdjson::dom::element_type::STRING)
        {
          if (failure)
          {
            failure->assign("applications[].name requires a string"_ctv);
          }
          return false;
        }
      ...
      {
        uint64_t valueU64 = 0;
        if ((field.value.type() != simdjson::dom::element_type::INT64 && field.value.type() != simdjson::dom::element_type::UINT64) || field.value.get(valueU64) != simdjson::SUCCESS || valueU64 == 0 || valueU64 > UINT32_MAX)
        {
          if (failure)
          {
            failure->assign("applications[].instances invalid"_ctv);
          }
          return false;
        }
      ...
      else
      {
        if (failure)
        {
          failure->snprintf<"applications[].{} is not recognized"_ctv>(key);
        }
        return false;
      }
    }
    ...
    if (application.instances == 0)
    {
      if (failure)
      {
        failure->assign("applications[].instances required"_ctv);
      }
      return false;
    }
    ...
    if (application.name.size() == 0)
    {
      if (failure)
      {
        failure->assign("applications[].name required"_ctv);
      }
      return false;
    }
    ...
    String *failure = nullptr)
{
  ...
  if (value.type() != simdjson::dom::element_type::ARRAY)
  {
    if (failure)
    {
      failure->assign("machines requires an array"_ctv);
    }
    return false;
  }
  ...
  for (auto item : value.get_array())
  {
    if (item.type() != simdjson::dom::element_type::OBJECT)
    {
      if (failure)
      {
        failure->assign("machines requires object members"_ctv);
      }
      return false;
    }
    ...
    for (auto field : item.get_object())
    {
      ...
      if (key.equal("providerMachineType"_ctv))
      {
        if (field.value.type() != simdjson::dom::element_type::STRING)
        {
          if (failure)
          {
            failure->assign("machines[].providerMachineType requires a string"_ctv);
          }
          return false;
        }
      ...
      else if (key.equal("kind"_ctv))
      {
        if (field.value.type() != simdjson::dom::element_type::STRING)
        {
          if (failure)
          {
            failure->assign("machines[].kind requires a string"_ctv);
          }
          return false;
        }
        ...
        else
        {
          if (failure)
          {
            failure->assign("machines[].kind invalid"_ctv);
          }
          return false;
        }
      }
      else if (key.equal("count"_ctv))
      {
        ...
        if ((field.value.type() != simdjson::dom::element_type::INT64 && field.value.type() != simdjson::dom::element_type::UINT64) || field.value.get(valueU64) != simdjson::SUCCESS || valueU64 == 0 || valueU64 > UINT32_MAX)
        {
          if (failure)
          {
            failure->assign("machines[].count invalid"_ctv);
          }
          return false;
        }
      ...
      else if (key.equal("storageGB"_ctv))
      {
        ...
        else
        {
          if (failure)
          {
            failure->assign("machines[].storageGB invalid"_ctv);
          }
          return false;
        }
      }
      else
      {
        if (failure)
        {
          failure->snprintf<"machines[].{} is not recognized"_ctv>(key);
        }
        return false;
      }
    }
    ...
    if (selection.providerMachineType.size() == 0)
    {
      if (failure)
      {
        failure->assign("machines[].providerMachineType required"_ctv);
      }
      return false;
    }
    ...
    if (selection.count == 0)
    {
      if (failure)
      {
        failure->assign("machines[].count required"_ctv);
      }
      return false;
    }
