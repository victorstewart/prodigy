...
static inline bool prodigyRunLocalShellCommand(const String& command, String *failure = nullptr)
{
  ...
  if (rc != 0)
  {
    if (failure)
    {
      failure->snprintf<"local command failed: {}"_ctv>(command);
    }
    return false;
  }
...
static inline bool prodigyComputeSHA256Hex(const uint8_t *data, uint64_t size, String& digest, String *failure = nullptr)
{
  ...
  if (data == nullptr && size > 0)
  {
    if (failure)
    {
      failure->assign("sha256 source buffer is null"_ctv);
    }
    return false;
  }
  ...
  if (ctx == nullptr)
  {
    if (failure)
    {
      failure->assign("EVP_MD_CTX_new failed"_ctv);
    }
    return false;
  }
  ...
  if (EVP_DigestInit_ex(ctx, EVP_sha256(), nullptr) != 1)
  {
    ...
    if (failure)
    {
      failure->assign("EVP_DigestInit_ex failed"_ctv);
    }
  }
  ...
  if (ok && size > 0 && EVP_DigestUpdate(ctx, data, size_t(size)) != 1)
  {
    ...
    if (failure)
    {
      failure->assign("EVP_DigestUpdate failed"_ctv);
    }
  }
  ...
  if (ok && EVP_DigestFinal_ex(ctx, rawDigest, &rawDigestSize) != 1)
  {
    ...
    if (failure)
    {
      failure->assign("EVP_DigestFinal_ex failed"_ctv);
    }
  }
  ...
  if (rawDigestSize != 32)
  {
    if (failure)
    {
      failure->snprintf<"unexpected sha256 size: {}"_ctv>(rawDigestSize);
    }
    digest.clear();
    return false;
  }
...
static inline bool prodigyLoadBundleExpectedSHA256Hex(const String& bundlePath, String& expectedDigest, String *failure = nullptr)
{
  ...
  if (prodigyFileReadable(sha256Path) == false)
  {
    if (failure)
    {
      failure->snprintf<"bundle sha256 sidecar is not readable: {}"_ctv>(sha256Path);
    }
    return false;
  }
  ...
  if (prodigyIsSHA256HexDigest(expectedDigest) == false)
  {
    if (failure)
    {
      failure->snprintf<"bundle sha256 sidecar is invalid: {}"_ctv>(sha256Path);
    }
    expectedDigest.clear();
    return false;
  }
...
static inline bool prodigyComputeFileSHA256Hex(const String& path, String& digest, uint64_t *actualBytes, String *failure = nullptr)
{
  ...
  if (prodigyFileReadable(path) == false)
  {
    if (failure)
    {
      failure->snprintf<"sha256 source is not readable: {}"_ctv>(path);
    }
    return false;
  }
  ...
  if (fd < 0)
  {
    if (failure)
    {
      failure->snprintf<"open failed for sha256 source: {} errno={}"_ctv>(path, errno);
    }
    return false;
  }
  ...
  while (ok)
  {
    ...
    if (bytesRead < 0)
    {
      ...
      if (failure)
      {
        failure->snprintf<"read failed for sha256 source: {} errno={}"_ctv>(path, errno);
      }
      break;
    }
...
static inline bool prodigyFileMatchesExpectedSHA256Hex(const String& path, const String& expectedDigest, String& actualDigest, String *failure = nullptr)
{
  ...
  if (prodigyIsSHA256HexDigest(expectedDigest) == false)
  {
    if (failure)
    {
      failure->snprintf<"expected sha256 is invalid: {}"_ctv>(expectedDigest);
    }
    return false;
  }
  ...
  if (actualDigest != expectedDigest)
  {
    if (failure)
    {
      failure->snprintf<"sha256 mismatch expected={} actual={} path={}"_ctv>(expectedDigest, actualDigest, path);
    }
    return false;
  }
    ...
    String *failure = nullptr)
{
  ...
  if (measuredBytes != expectedBytes)
  {
    if (failure)
    {
      failure->snprintf<"blob size mismatch expected={} actual={} path={}"_ctv>(expectedBytes, measuredBytes, path);
    }
    return false;
  }
...
static inline bool prodigyIsZstdFile(const String& path)
{
  ...
  return header.size() >= 4 && uint8_t(header[0]) == 0x28 && uint8_t(header[1]) == 0xB5 && uint8_t(header[2]) == 0x2F && uint8_t(header[3]) == 0xFD;
}
...
static inline bool prodigyResolveBundleHomeDirectory(String& bundleHome, String *failure = nullptr)
{
  ...
  if (failure)
  {
    failure->assign("unable to resolve user prodigy bundle directory from XDG_DATA_HOME or HOME"_ctv);
  }
  return false;
}
...
static inline bool prodigyResolveBuiltBundleArtifact(const String& localExecutablePath, MachineCpuArchitecture architecture, String& bundlePath, String *failure = nullptr)
{
  ...
  if (failure)
  {
    failure->snprintf<"built prodigy bundle artifact is not readable: {}"_ctv>(bundlePath);
  }
  return false;
}
...
static inline bool prodigyResolveInstalledBundleArtifact(MachineCpuArchitecture architecture, String& bundlePath, String *failure = nullptr)
{
  ...
  if (failure)
  {
    failure->snprintf<"installed prodigy bundle artifact is not readable for architecture={} path={}"_ctv>(
...
static inline bool prodigyResolvePreferredBootstrapBundleArtifact(const String& localExecutablePath, MachineCpuArchitecture architecture, const String& installRoot, String& bundlePath, String *failure = nullptr)
{
  ...
  String builtFailure = {};
  if (localExecutablePath.size() > 0 && prodigyResolveBuiltBundleArtifact(localExecutablePath, architecture, bundlePath, &builtFailure))
  {
    return true;
  }
...
static inline bool prodigyResolveBundleArtifactInput(const String& inputPath, MachineCpuArchitecture architecture, String& bundlePath, String *failure = nullptr)
{
  ...
  if (inputPath.size() == 0)
  {
    if (failure)
    {
      failure->assign("bundle or executable path required"_ctv);
    }
    return false;
  }
  ...
  if (prodigyFileReadable(inputPath) == false)
  {
    if (failure)
    {
      failure->snprintf<"bundle or executable path is not readable: {}"_ctv>(inputPath);
    }
    return false;
  }
