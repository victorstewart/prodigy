...
public:
  ...
  ScopedSSHD()
  {
    ...
    if (readFile(prodigyTestSSHDHostPrivateKeyPath(), hostPrivateKey) == false || readFile(prodigyTestSSHDHostPublicKeyPath(), hostPublicKey) == false || readFile(prodigyTestClientSSHPublicKeyPath(), clientPublicKey) == false)
    {
    ...
    if (writeFileWithMode(hostKeyPath, hostPrivateKey, 0600) == false || writeFileWithMode(authorizedKeysPath, clientPublicKey, 0600) == false)
    {
...
int main(void)
{
  ...
  suite.expect(probedResources.totalMemoryMB == 32'768, "probe_parse_valid_output_memory");
  suite.expect(probedResources.totalStorageMB == 204'800, "probe_parse_valid_output_storage");
