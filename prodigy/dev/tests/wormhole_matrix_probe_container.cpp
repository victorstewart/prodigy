...
namespace
{
   static constexpr const char *kOpenPayload = "wormhole-open";
   ...
   bool prepareListeners(std::string& failure)
   {
      ...
      for (const Wormhole& wormhole : neuronHub->parameters.wormholes)
      {
         if (wormhole.externalAddress.isNull()
            || wormhole.externalPort == 0
            || wormhole.containerPort == 0
            || (wormhole.layer4 != IPPROTO_UDP && wormhole.layer4 != IPPROTO_TCP))
         {
...
public:
   ...
   void prepare(int argc, char *argv[])
   {
      ...
      std::thread([this] { serveProbes(); }).detach();
   }
