...
public:
  ...
  void advertisementPairing(uint128_t, uint128_t, uint64_t, uint16_t, bool activate) override
  {
    if (activate)
    {
      advertisementActivateCalls += 1;
    }
    else
    {
      advertisementDeactivateCalls += 1;
    }
  }
...
constexpr static uint64_t meshService = 0xA11CEULL;
constexpr static uint16_t meshPort = 19'111;
...
static void testJumpConsistentHashStableAcrossThreadSeeds(TestSuite& suite)
{
  ...
  keys.push_back(uint128_t(11'329'350'464'516'745'216ULL) | (uint128_t(13'213'379'408'606'233'272ULL) << 64));
  keys.push_back(uint128_t(5'743'192'639'385'774'080ULL) | (uint128_t(14'767'483'417'386'435'866ULL) << 64));
...
static void testJumpConsistentHashStableAcrossDefaultGlobalSeed(TestSuite& suite)
{
  ...
  std::vector<uint128_t> keys;
  keys.push_back(uint128_t(211'519'444'849'527'808ULL) | (uint128_t(12'949'714'390'353'147'497ULL) << 64));
  keys.push_back(uint128_t(17'135'719'469'053'349'632ULL) | (uint128_t(10'903'082'970'765'322'110ULL) << 64));
  ...
  for (uint32_t seed = 1; seed <= nSeeds; ++seed)
  {
    ...
    for (uint32_t step = 0; step < nSteps; ++step)
    {
      ...
      else
      {
        ...
        {
          case 0:
            nextNature = SubscriptionNature::any;
            break;
          case 1:
            nextNature = SubscriptionNature::exclusiveSome;
            break;
          case 2:
            nextNature = SubscriptionNature::all;
            break;
          default:
            nextNature = SubscriptionNature::none;
            break;
        }
