...
int main(int argc, char *argv[])
{
  ...
  if ((role.equal("brain"_ctv) == false && role.equal("neuron"_ctv) == false) || controlSocketPath.size() == 0 || localIndex == 0 || nBrains == 0 || machineSpecs.empty())
  {
    printUsage(argv[0]);
    return EXIT_FAILURE;
  }

  if (peerFamily.size() > 0 && peerFamily.equal("ipv4"_ctv) == false && peerFamily.equal("private6"_ctv) == false && peerFamily.equal("public6"_ctv) == false && peerFamily.equal("multihome6"_ctv) == false)
  {
