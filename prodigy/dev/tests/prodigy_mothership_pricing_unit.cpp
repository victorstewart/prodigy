...
static ProviderMachineOffer makeProviderOffer(const String& type, MachineConfig::MachineKind kind, double hourlyUSD)
{
  ...
  offer.nMemoryMB = 32'768;
  offer.nStorageMB = 262'144;
    ...
    const String& providerCredentialName)
{
  ...
  {
    if (target.provider == provider && target.providerScope.equals(providerScope) && target.providerCredentialName.equals(providerCredentialName))
    {
      return true;
    }
  }
...
int main(void)
{
...

  {
    ...
    slowBareMetal.nicSpeedMbps = 1000;
...

  {
    ...
    suite.expect(decoded.storageMB == 524'288, "pricing_selection_roundtrip_storage");
  }
...

  {
    ...
    cpuSelection.storageMB = 102'400;
    ...
    gpuSelection.storageMB = 204'800;
...

  {
    ...
    offer.nStorageMBDefault = 30'720;
    offer.providesHostPublic4 = true;
    offer.freeTierEligible = true;
    offer.hourlyMicrousd = 12'345;
    ...
    suite.expect(decoded.hourlyMicrousd == 12'345, "pricing_offer_roundtrip_hourly_microusd");
  }
