...
int main(void)
{
  ...
  bool ready = mothershipWaitForLocalTestClusterReady(cluster, &failure, 5000);
  if (ready == false)
  {
  ...
  bool spacedReady = mothershipWaitForLocalTestClusterReady(spacedCluster, &failure, 5000);
  ...
  bool failingReady = mothershipWaitForLocalTestClusterReady(failingCluster, &failingReadyFailure, 5000);
