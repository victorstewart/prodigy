...
#define PRODIGY_PINGPONG_DEFAULT_PORT 19'090
#endif
...
private:
  ...
  constexpr static uint32_t maxLineBytes = 4096;
  constexpr static uint32_t initialClientBufferBytes = 4096;
  NeuronHub *metricsSink = nullptr;
  uint64_t requestMetricKey = 0;
  bool readyReassertedOnTraffic = false;
  bool ipv6Listener = false;
  ...
  bool probeGoogleAddress(const struct in_addr& googleAddress, String& detail)
  {
    ...
    constexpr static const char *request =
        "GET / HTTP/1.1\r\n"
        "Host: www.google.com\r\n"
        "Connection: close\r\n"
  ...
  bool performGoogleHttpProbeV4(String& detail)
  {
    ...
    constexpr static const char *fallbackIPs[] = {
        "142.250.191.4",
        "142.251.32.4"};
  ...
  void closeClient(TCPStream *client)
  {
    ...
    if (Ring::socketIsClosing(client))
    {
      return;
    }
    if (client->fslot < 0)
...
public:
  ...
  explicit PingPongServer(std::atomic<bool>& runningFlag)
      : running(runningFlag)
  {
  }
  ...
  void start()
  {
    listener.setIPVersion(ipv6Listener ? AF_INET6 : AF_INET);
#ifdef SO_REUSEPORT
...
private:
  ...
  void signalReadyAndSeedMetricOnce(void)
  {
    ...
    if (topologyCutoverBarrierPublished == false && topology.servingMode == StatefulTopologyServingMode::catchupOnly && topology.sourceEpoch != 0 && topology.targetEpoch != 0)
    {
...
public:
  ...
  bool prepareStorageHandoffFixture()
  {
    ...
    if (const char *waitForTraffic = getenv("PINGPONG_STORAGE_HANDOFF_WAIT_FOR_TRAFFIC"))
    {
      if (strcmp(waitForTraffic, "1") != 0) return fail("wait-for-traffic-format");
      // Only a real served request releases seed readiness through the server's
      // existing traffic readiness owner. Verify still requires full readback.
      storageHandoffSeedReadyWithheld = seed;
