...
static bool equalMachines(const Vector<MothershipProdigyClusterMachine>& lhs, const Vector<MothershipProdigyClusterMachine>& rhs)
{
  ...
  for (uint32_t index = 0; index < lhs.size(); ++index)
  {
    if (lhs[index].source != rhs[index].source || lhs[index].backing != rhs[index].backing || lhs[index].kind != rhs[index].kind || lhs[index].lifetime != rhs[index].lifetime || lhs[index].isBrain != rhs[index].isBrain || lhs[index].cloud != rhs[index].cloud || lhs[index].ssh != rhs[index].ssh || lhs[index].addresses != rhs[index].addresses || lhs[index].ownership != rhs[index].ownership || lhs[index].rackUUID != rhs[index].rackUUID)
    {
      return false;
    }
  }
...
int main(void)
{
...

  {
    ...
    // seed guest. Keep this literal V2 record independent from the V3 writer.
...

  {
    ...
    String encodedV3 = {};
    {
      ...
      bool readV3 = database.read("clusters"_ctv, "local-homelab"_ctv, encodedV3, &failure);
      suite.expect(readV3, "rack_v3_record_read");
      suite.expect(failure.size() == 0, "rack_v3_record_read_no_failure");
    }
    constexpr auto v3Header = "PRODIGY-MOTHERSHIP-CLUSTER\nversion=3\n\n"_ctv;
    bool hasV3Header = encodedV3.size() >= v3Header.size() &&
                       std::memcmp(encodedV3.data(), v3Header.data(), v3Header.size()) == 0;
    suite.expect(hasV3Header, "rack_v3_record_header");
    ...
    String malformedValue = v3Header;
    ...
    malformedDbPath.snprintf<"{}/malformed-v3"_ctv>(dbPath);
    {
      ...
      bool wroteMalformed = database.write("clusters"_ctv, "rack-v3-malformed"_ctv, malformedValue, &failure);
      suite.expect(wroteMalformed, "rack_v3_malformed_record_written");
