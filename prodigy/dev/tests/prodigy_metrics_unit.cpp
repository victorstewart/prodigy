...
int main(void)
{
  ...
  {
    Vector<float> out;
    store.collectValues(deploymentID, nullptr, metricKey, 10'000, 5000, true, out);
    suite.expect(out.size() == 0, "empty_collect_fleet");
  }

  // Insert base samples for two containers.
  store.record(deploymentID, containerA, metricKey, 1000, 10.0);
  store.record(deploymentID, containerA, metricKey, 2000, 20.0);
  store.record(deploymentID, containerB, metricKey, 2500, 30.0);
  store.record(deploymentID, containerB, metricKey, 3000, 40.0);

  {
    Vector<float> out;
    store.collectValues(deploymentID, nullptr, metricKey, 3000, 2000, true, out); // cutoff = 1'000
    suite.expect(out.size() == 4, "fleet_collect_count");
  }

  {
    Vector<float> out;
    store.collectValues(deploymentID, &containerA, metricKey, 3000, 2000, false, out); // cutoff = 1'000
  ...
  {
    Vector<float> out;
    store.collectValues(deploymentID, nullptr, metricKey, 3000, 500, true, out); // cutoff = 2'500
...

  {
    ...
    monotonicStore.record(deploymentID, containerA, metricKey, 9000, 2.0); // out-of-order timestamp
    Vector<float> out;
...

  {
    ...
    Vector<float> out;
    growthStore.collectValues(deploymentID, nullptr, metricKey, 8192, 8192, true, out);
    suite.expect(out.size() == 8192, "ring_growth_preserves_all_samples");
...

  {
    ...
    trimStore.trimRetention(9900, 2000); // cutoff = 7'900

    Vector<float> out;
    trimStore.collectValues(deploymentID, nullptr, metricKey, 9900, 10'000, true, out);
...

  {
    ...
    for (uint32_t i = 0; i < 20'000; ++i)
    {
      ...
      if ((i % 64) == 0)
      {
        wrapStore.trimRetention(nowMs, 2048);
      }
    }

    Vector<float> out;
    wrapStore.collectValues(deploymentID, nullptr, metricKey, 20'000, 2048, true, out);
    suite.expect(out.size() > 0 && out.size() <= (2048 + 64), "wraparound_collect_respects_window");
  }

  {
    Vector<float> out;
    store.collectValues(deploymentID, nullptr, metricKey, 3000, 2000, true, out);
...

  {
    ...
    makeDeterministicSamples(samples, 4096, 0xBADC0FFEEULL);
...

  {
    ...
    for (uint32_t index = 0; index < sampleCount; ++index)
    {
      ...
      sample.ms = 1'800'000'000'000 + int64_t(index);
