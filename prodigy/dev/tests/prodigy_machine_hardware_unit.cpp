...
int main(void)
{
...

  {
    ...
    {
      savedHomeText.assign(savedHome);
    }
    if (savedConfigHome != nullptr)
    {
      savedConfigHomeText.assign(savedConfigHome);
    }
    ...
    if (savedHome != nullptr)
    {
      ::setenv("HOME", savedHomeText.c_str(), 1);
    }
    else
    {
      ::unsetenv("HOME");
    ...
    if (savedConfigHome != nullptr)
    {
      ::setenv("XDG_CONFIG_HOME", savedConfigHomeText.c_str(), 1);
    }
    else
    {
...

  {
    ...
    suite.expect(memory.modules[0].sizeMB == 16'384, "parse_memory_module_size_mb");
    ...
    suite.expect(bandwidthMBps == 182'346, "parse_bw_mem_output_value");
  }
...

  {
    ...
    if (savedDevMode != nullptr)
    {
      savedDevModeText.assign(savedDevMode);
    }
    if (savedStorageMounts != nullptr)
    {
      savedStorageMountsText.assign(savedStorageMounts);
    }
    ...
    if (savedDevMode != nullptr)
    {
      ::setenv("PRODIGY_DEV_MODE", savedDevModeText.c_str(), 1);
    }
    else
    {
      ::unsetenv("PRODIGY_DEV_MODE");
    ...
    if (savedStorageMounts != nullptr)
    {
      ::setenv("PRODIGY_DEV_CONTAINER_STORAGE_MOUNTS", savedStorageMountsText.c_str(), 1);
    }
    else
    {
      ::unsetenv("PRODIGY_DEV_CONTAINER_STORAGE_MOUNTS");
    }
  }
...

  {
    ...
    suite.expect(nics[0].linkSpeedMbps == 10'000, "parse_ethtool_speed");
...

  {
    ...
    suite.expect(clusterMachine.totalMemoryMB == 32'768, "cluster_machine_total_memory_from_hardware");
    suite.expect(clusterMachine.totalStorageMB == 512'000, "cluster_machine_total_storage_from_hardware");
    suite.expect(clusterMachine.ownedLogicalCores == 6, "cluster_machine_owned_cores_reserved");
    suite.expect(clusterMachine.ownedMemoryMB == (32'768 - 4096), "cluster_machine_owned_memory_reserved");
    suite.expect(clusterMachine.ownedStorageMB == (512'000 - 4096), "cluster_machine_owned_storage_reserved");
...

  {
    ...
    hardware.memory.totalMB = 65'536;
    hardware.inventoryComplete = true;

    MachineDiskHardwareProfile disk = {};
    disk.sizeMB = 1'024'000;
    hardware.disks.push_back(disk);
    hardware.network.internet.attempted = true;
    ...
    machine.ownershipStorageMBCap = 204'800;
    ...
    suite.expect(machine.totalMemoryMB == 65'536, "machine_total_memory_from_hardware");
    suite.expect(machine.totalStorageMB == 1'024'000, "machine_total_storage_from_hardware");
    ...
    suite.expect(machine.ownedStorageMB == 204'800, "machine_owned_storage_hardcap");
...

  {
    ...
    machine.totalStorageMB = 51'200;
    machine.ownedLogicalCores = 0;
    machine.ownedMemoryMB = 4096;
    machine.ownedStorageMB = 47'104;
...

  {
    ...
    machine.hardware.memory.totalMB = 32'768;
...

  {
    ...
    machine.hardware.memory.totalMB = 65'536;
    ...
    suite.expect(topology.machines[0].hardware.memory.totalMB == 65'536, "authoritative_topology_transport_keeps_memory_summary");
