...
protected:
  ...
  void armMachineNeuronControl(Machine *machine) override
  {
    ...
    observedNeuronRegisteredBeforeArm = observedNeuronRegisteredBeforeArm && machine != nullptr && neurons.contains(&machine->neuron);
...
int main(void)
{
...

  {
    ...
    BrainView *peer = makePeer(uint128_t(0x90217), int64_t(1'234'567'890'000), private4, "10.0.0.217");
...

  {
    ...
    suite.expect(peer->attemptDeadlineMs == 12'345, "brain_close_handler_connector_inert_master_claim_preserves_attempt_deadline");
...

  {
    ...
    bool parsedEchoedNonce = (queuedArgs && Message::extractArg<ArgumentNature::fixed>(queuedArgs, echoedResponse) && Message::extractArg<ArgumentNature::fixed>(queuedArgs, echoedNonce));
...

  {
    ...
    if (installed)
    {
      ...
      bool parsedQueuedNonce = (queuedArgs && Message::extractArg<ArgumentNature::fixed>(queuedArgs, queuedResponse) && Message::extractArg<ArgumentNature::fixed>(queuedArgs, queuedNonce));
  ...
  withUniqueMothershipSocket("registration_self_master_claim_echo_socket_dir_created", [&] {
    ...
    forEachMessageInBuffer(peer->wBuffer, [&](Message *queued) {
      ...
      if (Message::extractArg<ArgumentNature::fixed>(args, echoedUUID) && Message::extractArg<ArgumentNature::fixed>(args, echoedBoottimens) && Message::extractArg<ArgumentNature::fixed>(args, echoedVersion) && Message::extractArg<ArgumentNature::fixed>(args, queuedExistingMasterUUID))
      {
        sawRegistration = true;
  ...
  withUniqueMothershipSocket("self_elect_as_master_rearms_connector_peers_socket_dir_created", [&] {
    ...
    inactiveConnectorPeer->attemptDeadlineMs = 12'345;
  ...
  withUniqueMothershipSocket("self_elect_as_master_recovers_pending_deployments_socket_dir_created", [&] {
    ...
    DeploymentPlan plan = makeDeploymentPlan(61'000, 1);
  ...
  withUniqueMothershipSocket("spin_application_queues_new_deployment_behind_unstarted_baseline_socket_dir_created", [&] {
    ...
    DeploymentPlan baselinePlan = makeDeploymentPlan(61'003, 1);
    DeploymentPlan nextPlan = makeDeploymentPlan(61'003, 2);
...

  {
    ...
    DeploymentPlan plan = makeDeploymentPlan(61'004, 1);
...

  {
    ...
    suite.expect(peer->attemptDeadlineMs == 12'345, "brain_close_handler_connector_inert_duplicate_preserves_attempt_deadline");
