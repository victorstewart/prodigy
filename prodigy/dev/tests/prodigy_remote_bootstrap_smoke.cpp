...
int main(void)
{
  ...
  if (requireEnv("PRODIGY_REMOTE_BOOTSTRAP_MASTER_IP", masterIP) == false || requireEnv("PRODIGY_REMOTE_BOOTSTRAP_TARGET_IP", targetIP) == false || requireEnv("PRODIGY_REMOTE_BOOTSTRAP_SSH_KEY", sshKeyPath) == false || requireEnv("PRODIGY_REMOTE_BOOTSTRAP_SEED_KEY", bootstrapSeedKeyPath) == false || requireEnv("PRODIGY_REMOTE_BOOTSTRAP_REMOTE_SEED_KEY", remoteBootstrapSeedKeyPath) == false || requireEnv("PRODIGY_REMOTE_BOOTSTRAP_REMOTE_ROOT", remoteRoot) == false || requireEnv("PRODIGY_REMOTE_BOOTSTRAP_STATE_DIR", stateDir) == false || requireEnv("PRODIGY_REMOTE_BOOTSTRAP_SYSTEMD_DIR", systemdDir) == false || requireEnv("PRODIGY_REMOTE_BOOTSTRAP_SYSTEMCTL_LOG", systemctlLogPath) == false)
  {
    return EXIT_FAILURE;
  }
