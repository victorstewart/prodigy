...
detect_execute_arch()
{
   case "$(uname -m)" in
      x86_64|amd64)
         ...
         ;;
      aarch64|arm64)
         execute_arch="aarch64"
         ;;
      riscv64|riscv)
         execute_arch="riscv64"
         ;;
      *)
         echo "FAIL: unsupported host architecture for whitehole smoke: $(uname -m)"
         exit 1
         ;;
   esac
}
   ...
   cat > "${metadata_dir}/launch.metadata" <<EOF
{
  ...
  "execute_env": [],
  "execute_cwd": "/",
  "execute_arch": "${execute_arch_name}"
}
EOF
...
copy_binary_and_libs_into_subvol "${WHITEHOLE_BIN}" "${deployment_subvol}" "/root/whitehole_probe_container" "${execute_arch}"

cat > "${deployment_subvol}/rootfs/etc/hosts" <<EOF
127.0.0.1 localhost
...
do
   if [[ -f "${stdout_log}" ]] && rg -q 'probe\.(success|fail)' "${stdout_log}"
   then
...
then
   archive_workspace=1
   echo "FAIL: whitehole probe container did not report success"
...
then
   archive_workspace=1
   echo "FAIL: spoofed reply reached the whitehole container"
   sed -n '1,160p' "${probe_log}" || true
   exit 1
fi
