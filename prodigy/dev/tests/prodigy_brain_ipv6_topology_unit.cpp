...
int main(void)
{
...

  {
    ...
    ClusterTopology topology = {};
    topology.version = 7;
    topology.machines.push_back(makeMultihomedBrainMachine(neuron.uuid, {
                                                                            {"fd00:10::10", 64}
    }));
    topology.machines.push_back(makeMultihomedBrainMachine(0x444, {
                                                                      {"fd00:10::29",  64},
                                                                      {"2001:db8::29", 64}
    }));
...

  {
    ...
    ClusterTopology topology = {};
    topology.machines.push_back(makeMultihomedBrainMachine(0x701, {
                                                                      {"fd00:10::150",  64},
                                                                      {"2001:db8::150", 64}
    }));
...

  {
    ...
    brain.storedTopology.machines.push_back(makeMultihomedBrainMachine(neuron.uuid, {
                                                                                        {"2001:db8::10", 64}
    }));
...

  {
    ...
    if (selfMachine != nullptr)
    {
      ...
      suite.expect(selfMachine->totalStorageMB == 20'480, "restore_machine_replays_local_hardware_inventory_storage");
...

  {
    ...
    if (selfMachine != nullptr)
    {
      ...
      suite.expect(selfMachine->totalStorageMB == 40'960, "restore_machine_polls_deferred_local_hardware_inventory_storage");
...

  {
    ...
    if (selfMachine != nullptr)
    {
      ...
      suite.expect(selfMachine->totalStorageMB == 30'720, "restore_machine_accepts_late_local_hardware_inventory_storage");
...

  {
    ...
    forEachMessageInBuffer(active.neuron.wBuffer, [&](Message *message) {
      ...
      if (BitseryEngine::deserializeSafe(payload, decoded) && decoded.size() == 1 && decoded[0].uuid == subnet.uuid && decoded[0].subnet.cidr == 64)
      {
...

  {
    ...
    forEachMessageInBuffer(local.neuron.wBuffer, [&](Message *message) {
      ...
      if (BitseryEngine::deserializeSafe(payload, decoded) && decoded.containerNetworkViaOverlay && decoded.overlaySubnets.size() == 1 && decoded.machineRoutes.size() == 1 && decoded.machineRoutes[0].machineFragment == remote.fragment && decoded.hostedIngressRoutes.size() == 1 && decoded.hostedIngressRoutes[0].machineFragment == remote.fragment)
      {
