...
static ClusterMachine makeTopologyMachine(const String& schema, const String& privateAddress, bool isBrain)
{
  ...
  machine.totalMemoryMB = 16'384;
  machine.totalStorageMB = 131'072;
  machine.ownedLogicalCores = 8;
  machine.ownedMemoryMB = 16'384;
  machine.ownedStorageMB = 131'072;
  return machine;
}
...
int main(void)
{
  ...
  MothershipProdigyCluster storedTest = {};
  {
    ...
    if (!createdLocal)
    {
      basics_log("detail create_local_cluster: %s\n", failure.c_str());
    }
    ...
    if (!createdTest)
    {
      basics_log("detail create_test_cluster: %s\n", failure.c_str());
    }
    ...
    if (!listedSeeded)
    {
      basics_log("detail list_seeded_clusters: %s\n", failure.c_str());
    }
  ...
  reportedMachine.totalMemoryMB = 16'384;
  reportedMachine.totalStorageMB = 131'072;
  reportedMachine.ownedLogicalCores = 8;
  reportedMachine.ownedMemoryMB = 16'384;
  reportedMachine.ownedStorageMB = 131'072;
  ...
  machineReport.totalMemoryMB = 16'384;
  machineReport.totalStorageMB = 131'072;
  machineReport.ownedLogicalCores = 8;
  machineReport.ownedMemoryMB = 16'384;
  machineReport.ownedStorageMB = 131'072;
  ...
  MothershipProdigyCluster refreshedLocalAfterSuccess = {};
  {
    ...
    if (!loaded)
    {
      basics_log("detail load_cluster_after_cluster_report_success: %s\n", failure.c_str());
    }
  ...
  MothershipProdigyCluster refreshedLocalAfterFailure = {};
  {
    ...
    if (!loaded)
    {
      basics_log("detail load_cluster_after_cluster_report_failure: %s\n", failure.c_str());
    }
