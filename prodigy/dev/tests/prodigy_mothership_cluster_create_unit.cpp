...
public:
  ...
  bool prepareProviderBootstrapArtifacts(const MothershipProdigyCluster& cluster, ProdigyTimingAttribution *timingAttribution = nullptr, String *failure = nullptr) override
  {
    ...
    if (failPrepareProviderBootstrapArtifacts)
    {
      if (failure)
      {
        failure->assign("prepare provider bootstrap artifacts failed"_ctv);
      }
      return false;
    }
  ...
  bool destroyCreatedSeedMachine(const MothershipProdigyCluster& cluster, const ClusterMachine& seedMachine, String *failure = nullptr) override
  {
    ...
    if (failDestroyCreatedSeed)
    {
      if (failure)
      {
        failure->assign("destroy seed failed"_ctv);
      }
      return false;
    }
  ...
  bool bootstrapRemoteSeed(const MothershipProdigyCluster& cluster, const ClusterMachine& seedMachine, const AddMachines& request, const ClusterTopology& topology, const ProdigyRuntimeEnvironmentConfig& runtimeEnvironment, ProdigyTimingAttribution *timingAttribution = nullptr, String *failure = nullptr) override
  {
    ...
    if (failBootstrapRemoteSeed)
    {
      if (failure)
      {
        failure->assign("bootstrap remote seed failed"_ctv);
      }
      return false;
    }
  ...
  bool configureSeedCluster(const MothershipProdigyCluster& cluster, const BrainConfig& config, String *failure = nullptr) override
  {
    ...
    if (failConfigureSeedCluster)
    {
      if (failure)
      {
        failure->assign("configure seed failed"_ctv);
      }
      return false;
    }
  ...
  bool fetchSeedTopology(const MothershipProdigyCluster& cluster, ClusterTopology& topology, String *failure = nullptr) override
  {
    ...
    if (failFetchSeedTopology)
    {
      if (failure)
      {
        failure->assign("fetch seed topology failed"_ctv);
      }
      return false;
    }
  ...
  bool applyAddMachines(const MothershipProdigyCluster& cluster, const AddMachines& request, ClusterTopology& topology, ProdigyTimingAttribution *timingAttribution = nullptr, String *failure = nullptr) override
  {
    ...
    if (failApplyAddMachines)
    {
      if (failure)
      {
        failure->assign("apply addMachines failed"_ctv);
      }
      return false;
    }
  ...
  bool upsertMachineSchemas(const MothershipProdigyCluster& cluster, const Vector<ProdigyManagedMachineSchemaPatch>& patches, ClusterTopology& topology, ProdigyTimingAttribution *timingAttribution = nullptr, String *failure = nullptr) override
  {
    ...
    if (failUpsertMachineSchemas)
    {
      if (failure)
      {
        failure->assign("upsert machine schemas failed"_ctv);
      }
      return false;
    }
...
int main(void)
{
...

  {
    ...
    appendClusterMachineConfig(cluster, makeMachineConfig("aws-brain-vm"_ctv, MachineConfig::MachineKind::vm, 4, 16'384, 131'072));
    ...
    suite.expect(seedMachine.totalStorageMB == 65'536, "seed_provision_total_storage_from_snapshot");
...

  {
    ...
    appendClusterMachineConfig(cluster, makeMachineConfig("local-dev"_ctv, MachineConfig::MachineKind::bareMetal, 8, 16'384, 131'072));
    ...
    suite.expect(equalCallSequence(hooks.callSequence, {ClusterCreateCall::localBootstrap,
...

  {
    ...
    appendClusterMachineConfig(cluster, makeMachineConfig("homelab-brain"_ctv, MachineConfig::MachineKind::vm, 8, 16'384, 131'072));
    appendClusterMachineConfig(cluster, makeMachineConfig("homelab-worker"_ctv, MachineConfig::MachineKind::vm, 4, 8192, 65'536));
    cluster.nBrains = 3;
    ...
    cluster.bgp.config.peers.push_back(makeBGPConfigPeer("169.254.1.1"_ctv, "10.0.0.21"_ctv, 64'512, "homelab-md5-v4"_ctv, 2));
    cluster.bgp.config.peers.push_back(makeBGPConfigPeer("2001:19f0:ffff::1"_ctv, "2001:db8::21"_ctv, 64'512, "homelab-md5-v6"_ctv, 2));
...

  {
    ...
    cluster.controls.push_back(makeUnixControl("/run/prodigy/control.sock"_ctv));
    appendClusterMachineConfig(cluster, makeMachineConfig("gcp-brain"_ctv, MachineConfig::MachineKind::vm, 4, 16'384, 131'072));
    appendClusterMachineConfig(cluster, makeMachineConfig("gcp-worker"_ctv, MachineConfig::MachineKind::vm, 4, 8192, 65'536));
    cluster.nBrains = 3;
    cluster.bootstrapSshUser = "root"_ctv;
    cluster.bootstrapSshPrivateKeyPath = prodigyTestBootstrapSeedSSHPrivateKeyPath();
    cluster.remoteProdigyPath = "/root/prodigy"_ctv;
    ...
    suite.expect(equalCallSequence(hooks.callSequence, {ClusterCreateCall::remoteBootstrap,
                                                        ClusterCreateCall::configure,
                                                        ClusterCreateCall::fetchTopology,
                                                        ClusterCreateCall::applyAddMachines}),
...

  {
    ...
    cluster.clusterUUID = 0x10031003;
    cluster.deploymentMode = MothershipClusterDeploymentMode::remote;
    cluster.provider = MothershipClusterProvider::gcp;
    ...
    suite.expect(equalCallSequence(hooks.callSequence, {ClusterCreateCall::prepareProviderBootstrapArtifacts,
...

  {
    ...
    cluster.controls.push_back(makeUnixControl("/run/prodigy/control.sock"_ctv));
    appendClusterMachineConfig(cluster, makeMachineConfig("aws-brain-vm"_ctv, MachineConfig::MachineKind::vm, 4, 16'384, 131'072));
    appendClusterMachineConfig(cluster, makeMachineConfig("aws-worker-vm"_ctv, MachineConfig::MachineKind::vm, 8, 32'768, 262'144));
    cluster.nBrains = 3;
...

  {
    ...
    cluster.controls.push_back(makeUnixControl("/run/prodigy/control.sock"_ctv));
    appendClusterMachineConfig(cluster, makeMachineConfig("azure-brain-vm"_ctv, MachineConfig::MachineKind::vm, 2, 8192, 131'072));
    cluster.nBrains = 3;
    cluster.bootstrapSshUser = "root"_ctv;
    cluster.bootstrapSshPrivateKeyPath = prodigyTestBootstrapSeedSSHPrivateKeyPath();
    cluster.remoteProdigyPath = "/root/prodigy"_ctv;
...

  {
    ...
    cluster.clusterUUID = 0x10041004;
    cluster.deploymentMode = MothershipClusterDeploymentMode::remote;
    ...
    cluster.controls.push_back(makeUnixControl("/run/prodigy/control.sock"_ctv));
    appendClusterMachineConfig(cluster, makeMachineConfig("vultr-brain-vm"_ctv, MachineConfig::MachineKind::vm, 4, 16'384, 131'072));
    appendClusterMachineConfig(cluster, makeMachineConfig("vultr-worker-vm"_ctv, MachineConfig::MachineKind::vm, 4, 8192, 65'536));
    cluster.nBrains = 3;
    cluster.bootstrapSshUser = "root"_ctv;
    cluster.bootstrapSshPrivateKeyPath = prodigyTestBootstrapSeedSSHPrivateKeyPath();
    cluster.remoteProdigyPath = "/root/prodigy"_ctv;
    ...
    cluster.bgp.config.peers.push_back(makeBGPConfigPeer("169.254.1.1"_ctv, "10.9.0.21"_ctv, 64'512, "vultr-md5-v4"_ctv, 2));
    cluster.bgp.config.peers.push_back(makeBGPConfigPeer("2001:19f0:ffff::1"_ctv, "2001:db8:9::21"_ctv, 64'512, "vultr-md5-v6"_ctv, 2));
...

  {
    ...
    appendClusterMachineConfig(cluster, makeMachineConfig("azure-brain-vm"_ctv, MachineConfig::MachineKind::vm, 4, 16'384, 131'072));
    appendClusterMachineConfig(cluster, makeMachineConfig("azure-worker-vm"_ctv, MachineConfig::MachineKind::vm, 4, 8192, 65'536));
    cluster.nBrains = 3;
    cluster.bootstrapSshUser = "root"_ctv;
    cluster.bootstrapSshPrivateKeyPath = prodigyTestBootstrapSeedSSHPrivateKeyPath();
    cluster.remoteProdigyPath = "/root/prodigy"_ctv;
...

  {
    ...
    cluster.propagateProviderCredentialToProdigy = true;
    cluster.controls.push_back(makeUnixControl("/run/prodigy/control.sock"_ctv));
    appendClusterMachineConfig(cluster, makeMachineConfig("vultr-brain"_ctv, MachineConfig::MachineKind::vm, 4, 16'384, 131'072));
    appendClusterMachineConfig(cluster, makeMachineConfig("vultr-worker"_ctv, MachineConfig::MachineKind::vm, 4, 8192, 65'536));
    cluster.nBrains = 3;
    cluster.bootstrapSshUser = "root"_ctv;
    cluster.bootstrapSshPrivateKeyPath = prodigyTestBootstrapSeedSSHPrivateKeyPath();
    cluster.remoteProdigyPath = "/root/prodigy"_ctv;
...
#if PRODIGY_ENABLE_CREATE_TIMING_ATTRIBUTION
  {
    ...
    suite.expect(
        timingSummary.total.providerWaitNs == hooks.createSeedTiming.providerWaitNs + hooks.remoteBootstrapTiming.providerWaitNs + hooks.addMachinesTiming.providerWaitNs + hooks.upsertMachineSchemasTiming.providerWaitNs,
        "create_remote_attribution_total_provider_wait");
    suite.expect(
        timingSummary.total.runtimeOwnedNs >= hooks.createSeedTiming.runtimeOwnedNs + hooks.remoteBootstrapTiming.runtimeOwnedNs + hooks.addMachinesTiming.runtimeOwnedNs + hooks.upsertMachineSchemasTiming.runtimeOwnedNs,
