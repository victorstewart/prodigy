    ...
    MachineLifetime lifetime)
{
  ...
  machine.totalStorageMB = 262'144;
  machine.ownedLogicalCores = 8;
  machine.ownedMemoryMB = 16'384;
  machine.ownedStorageMB = 262'144;
  return machine;
}
...
static bool handleClusterReportStep(int clientFD, ControlServerState& state, String& failure)
{
  ...
  report.machineReports.back().activeStorageMB = 16'384;
    ...
    String& failure)
{
  ...
  state.deployBlobMatched = (blobSize == expectedBlob.size() && (blobSize == 0 || std::memcmp(blobBytes, expectedBlob.data(), blobSize) == 0));
...
int main(void)
{
...

  {
    ...
    if (ranOversizedClusterReport == false || oversizedClusterReportExitCode != EXIT_FAILURE || stringContains(oversizedClusterReportOutput, "framed message exceeds control-frame limit") == false || oversizedServer.acceptCount != 1 || oversizedServer.sawPullClusterReport == false || oversizedServer.failure.size() > 0)
    {
  ...
  if (executableOnPath("mkfs.btrfs"))
  {
    ...
    const uint16_t deployApplicationID = 62'011;
