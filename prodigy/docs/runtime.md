...
Network access is enforced in the provider's private netns by the container egress router. Runtime launch accepts only a public literal IPv4 TCP endpoint and loads that exact tuple into a per-container BPF allowlist. The provider receives `PRODIGY_TUNNEL_EGRESS_HOST` and `PRODIGY_TUNNEL_EGRESS_PORT` from that endpoint. Provider resource requests are capped at 16 logical cores, 32 GiB memory, and 256 GiB storage.

If a tunnel is misconfigured, normal cluster commands fail closed instead of downgrading to SSH. Recovery is out-of-band host/provider access or a corrected tunnel spec through an available control path.

Create-time seed requests use the bootstrap control path until the tunnel-provider config has been delivered. The seed-configure path sends Brain the verified system artifact, gateway auth, and secret-free runtime connectivity config. Brain persists and replicates gateway auth and runtime connectivity outside `BrainConfig`. The concrete Prodigy Brain runtime hook owns `/run/prodigy/mothership-tunnel-gateway.sock`, authenticates gateway sessions before opening the raw control socket, marks tunnel health after that socket opens, restricts peers to the provider UID, and stops the runtime on spec changes, SSH cutover, master loss, or provider terminal failure.
