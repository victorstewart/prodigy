...
Tunnel-provider cluster metadata must also include generated gateway client auth material for the external `mothership` process: a dedicated root certificate, client certificate, and client private key. Create-time seed configure sends Brain one desired state containing the runtime connectivity shape and gateway-side root/server keys. Brain stores that desired state in `ProdigyMasterAuthorityRuntimeState`; persistent snapshot scrubbing keeps the gateway server key in the snapshot secret sidecar, while client auth private keys stay in the local cluster registry. Runtime reconciliation refuses to advance toward provider launch until Brain has configured gateway auth.
