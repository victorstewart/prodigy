...
Cluster records may choose `mothershipConnectivity.kind=ssh` or `tunnelProvider`. `ssh` is the compatibility default. `tunnelProvider` is a system-container contract, not an application deployment and not `ApplicationType::tunnel`.

Emergency SSH fallback for a `tunnelProvider` cluster is never implicit: the cluster metadata must allow it and the caller must explicitly request it before the resolver enters the legacy SSH path. Operator recovery uses `PRODIGY_MOTHERSHIP_EMERGENCY_SSH_FALLBACK=1` on the single command process; without `allowEmergencySshFallback=true` in the cluster record, the resolver still refuses SSH.
