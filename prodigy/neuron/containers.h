...
// source proof.
constexpr static enum bpf_attach_type prodigyContainerIngressNetkitAttachType()
{
  return BPF_NETKIT_PRIMARY;
}

constexpr static enum bpf_attach_type prodigyContainerEgressNetkitAttachType()
{
...
static bool prodigyOverlayPrefix6KeyLess(const switchboard_overlay_prefix6_key& lhs, const switchboard_overlay_prefix6_key& rhs)
{
  if (lhs.prefixlen != rhs.prefixlen)
  {
    return lhs.prefixlen < rhs.prefixlen;
...
static void prodigySyncOverlayPresenceMap(BPFProgram *program,
                                          MapName&& mapName,
                                          ProdigyOverlayPresenceMirror<Key>& installed,
                                          Vector<Key>& desiredKeys,
                                          Less&& less)
{
  std::sort(desiredKeys.begin(), desiredKeys.end(), less);
...
public:
  ...
  void disableKillSwitch(void)
  {
    if (killSwitch)
    {
      Ring::queueCancelTimeout(killSwitch);
    }
  }
  ...
  bool setupNetwork(String *failureReport = nullptr)
  {
    ...
    if (hostnetnsfd < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open host netns for container {itoa}: {}"_ctv>(plan.uuid, String(strerror(errno)));
      }
    ...
    if (peernetnsfd < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open peer netns for container {itoa}: {}"_ctv>(plan.uuid, String(strerror(errno)));
      }
...
private:
  ...
  constexpr static uint64_t maxCompressedContainerBlobBytes = prodigyContainerRuntimeLimits.maxCompressedBlobBytes;
  constexpr static uint64_t maxContainerArtifactRegularFileBytes = prodigyContainerRuntimeLimits.maxArtifactBytes;
  constexpr static uint64_t maxLaunchMetadataBytes = prodigyContainerRuntimeLimits.maxLaunchMetadataBytes;
  constexpr static uint64_t maxPendingCreateMarkerBytes = prodigyContainerRuntimeLimits.maxPendingCreateMarkerBytes;
  constexpr static uint64_t maxLaunchMetadataStringBytes = prodigyContainerRuntimeLimits.maxLaunchMetadataEntryBytes;
  constexpr static uint64_t maxLaunchMetadataPathBytes = prodigyContainerRuntimeLimits.maxLaunchMetadataPathBytes;
  constexpr static uint64_t maxLaunchMetadataArchBytes = prodigyContainerRuntimeLimits.maxLaunchMetadataArchitectureBytes;
  constexpr static uint32_t maxLaunchMetadataArgs = prodigyContainerRuntimeLimits.maxLaunchMetadataArrayEntries;
  constexpr static uint32_t maxLaunchMetadataEnv = prodigyContainerRuntimeLimits.maxLaunchMetadataArrayEntries;
  constexpr static uint32_t maxContainerArtifactEntries = prodigyContainerRuntimeLimits.maxArtifactEntries;
  ...
  static bool readOpenFileDescriptorIntoString(int fd, String& output, uint64_t maxBytes, String *failureReport = nullptr)
  {
    ...
    if (fd < 0)
    {
      if (failureReport)
      {
        failureReport->assign("file descriptor is invalid"_ctv);
      }
      return false;
    }
    ...
    if (fstat(fd, &statbuf) != 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"fstat failed errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      return false;
    }
    ...
    if (S_ISREG(statbuf.st_mode) == 0)
    {
      if (failureReport)
      {
        failureReport->assign("file descriptor does not reference a regular file"_ctv);
      }
      return false;
    }
    ...
    if (lseek(fd, 0, SEEK_SET) < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"lseek failed errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      return false;
    }
    ...
    if (maxBytes > 0 && fileBytes > maxBytes)
    {
      if (failureReport)
      {
        failureReport->snprintf<"file exceeds maximum size {} bytes"_ctv>(maxBytes);
      }
      return false;
    }
    ...
    while (true)
    {
      if (output.remainingCapacity() == 0)
      {
        ...
        if (output.need(growBy) == false)
        {
          if (failureReport)
          {
            failureReport->assign("failed to reserve buffer for file read"_ctv);
          }
          return false;
        }
      }
      ...
      if (bytesRead < 0)
      {
        ...
        if (failureReport)
        {
          failureReport->snprintf<"read failed errno={}({})"_ctv>(errno, String(strerror(errno)));
        }
        output.clear();
        return false;
      }
  ...
  static bool validateNormalizedAbsoluteContainerPath(const String& path, const String& fieldName, bool allowRoot, String *failureReport = nullptr)
  {
    if (path.size() == 0 || path[0] != '/')
    {
      if (failureReport)
      {
        failureReport->snprintf<"launch metadata {} must be absolute"_ctv>(fieldName);
      }
      return false;
    }
    ...
    if (path.size() == 1)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"launch metadata {} must not resolve to the container root directory"_ctv>(fieldName);
      }
      return false;
    }
    ...
    while (componentStart < path.size())
    {
      ...
      if (componentSize == 0)
      {
        if (failureReport)
        {
          failureReport->snprintf<"launch metadata {} must be normalized and must not contain empty path components"_ctv>(fieldName);
        }
        return false;
      }
      ...
      if (componentSize == 1 && path[componentStart] == '.')
      {
        if (failureReport)
        {
          failureReport->snprintf<"launch metadata {} must not contain '.' path components"_ctv>(fieldName);
        }
        return false;
      }
    ...
    if (path[path.size() - 1] == '/')
    {
      if (failureReport)
      {
        failureReport->snprintf<"launch metadata {} must be normalized and must not end with '/'"_ctv>(fieldName);
      }
      return false;
    }
  ...
  constexpr static uint64_t containerExecutionResolveFlags =
      ...
      String *failureReport = nullptr)
  {
    ...
    if (rootfd < 0)
    {
      if (failureReport)
      {
        failureReport->assign("container rootfs fd is invalid"_ctv);
      }
      return false;
    }
  ...
  static bool validateContainerLaunchTargetsInRootfs(Container *container, int rootfd, String *failureReport = nullptr)
  {
    if (container == nullptr)
    {
      if (failureReport)
      {
        failureReport->assign("container is null"_ctv);
      }
      return false;
    }
    ...
    bool executeValid = (fstat(executeFD, &executeStat) == 0 && S_ISREG(executeStat.st_mode) != 0 && (executeStat.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) != 0);
  ...
  static bool loadContainerLaunchMetadata(Container *container, String *failureReport = nullptr)
  {
    ...
    if (privateFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open container private metadata directory without following symlinks errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
    ...
    if (metadataFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open launch.metadata without following symlinks errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      close(privateFD);
      close(artifactRootFD);
      return false;
    ...
    simdjson::padded_string paddedMetadata(
        ...
        metadataJSON.size());
    if (parser.parse(paddedMetadata).get(doc) != simdjson::SUCCESS || doc.type() != simdjson::dom::element_type::OBJECT)
    {
      if (failureReport)
      {
    ...
    for (auto field : doc.get_object())
    {
      ...
      if (key == "execute_path"_ctv)
      {
        ...
        if (container->executePath.size() > maxLaunchMetadataPathBytes)
        {
          if (failureReport)
          {
            failureReport->snprintf<"launch metadata execute_path must be at most {itoa} bytes"_ctv>(uint32_t(maxLaunchMetadataPathBytes));
          }
          return false;
        }
        sawExecutePath = true;
      ...
      else if (key == "execute_cwd"_ctv)
      {
        ...
        if (container->executeCwd.size() > maxLaunchMetadataPathBytes)
        {
          if (failureReport)
          {
            failureReport->snprintf<"launch metadata execute_cwd must be at most {itoa} bytes"_ctv>(uint32_t(maxLaunchMetadataPathBytes));
          }
          return false;
        }
        sawExecuteCwd = true;
      ...
      else if (key == "execute_arch"_ctv)
      {
        ...
        if (executeArchText.size() > maxLaunchMetadataArchBytes)
        {
          if (failureReport)
          {
            failureReport->snprintf<"launch metadata execute_arch must be at most {itoa} bytes"_ctv>(uint32_t(maxLaunchMetadataArchBytes));
          }
          return false;
        }
        if (parseMachineCpuArchitecture(executeArchText, container->executeArchitecture) == false || prodigyMachineCpuArchitectureSupportedTarget(container->executeArchitecture) == false)
        {
          if (failureReport)
          {
    ...
    if (container->executePath.size() == 0 || container->executePath[0] != '/')
    {
      if (failureReport)
      {
        failureReport->assign("launch metadata execute_path must be absolute"_ctv);
      }
      return false;
    }
    ...
    if (container->executeCwd.size() == 0 || container->executeCwd[0] != '/')
    {
      if (failureReport)
      {
        failureReport->assign("launch metadata execute_cwd must be absolute"_ctv);
      }
      return false;
    }
    ...
    if (container->plan.config.architecture != MachineCpuArchitecture::unknown && container->plan.config.architecture != container->executeArchitecture)
    {
      if (failureReport)
      {
    ...
    if (localArchitecture != MachineCpuArchitecture::unknown && localArchitecture != container->executeArchitecture)
    {
      if (failureReport)
      {
        String executeArchitectureName = {};
...
private:
  ...
  static bool runExternalCommand(const char *label, const char *program, std::vector<char *>& argv, String *capturedStdout = nullptr, String *failureReport = nullptr)
  {
    ...
    if (program == nullptr || argv.empty() || argv.back() != nullptr)
    {
      if (failureReport)
      {
        failureReport->assign("invalid external command arguments"_ctv);
      }
      return false;
    }
    ...
    if (capturing)
    {
      if (pipe(stdoutPipe) != 0)
      {
        if (failureReport)
        {
          failureReport->snprintf<"{} pipe failed: {}"_ctv>(labelText, String(strerror(errno)));
        }
        return false;
      }
    ...
    if (rc != 0)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"{} spawn failed: {}"_ctv>(labelText, String(strerror(rc)));
      }
      return false;
    }
    ...
    while (waitpid(pid, &status, 0) < 0)
    {
      if (errno != EINTR)
      {
        if (failureReport)
        {
          failureReport->snprintf<"{} waitpid failed: {}"_ctv>(labelText, String(strerror(errno)));
        }
        return false;
      }
    }
  ...
  static bool listDirectoryEntriesAt(int directoryFD, Vector<String>& entries, String *failureReport = nullptr)
  {
    ...
    if (directoryFD < 0)
    {
      if (failureReport)
      {
        failureReport->assign("directory fd is invalid"_ctv);
      }
      return false;
    }
    ...
    if (iteratorFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to duplicate directory fd errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      return false;
    }
    ...
    if (dir == nullptr)
    {
      if (failureReport)
      {
        failureReport->snprintf<"fdopendir failed errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      close(iteratorFD);
      return false;
    }
    ...
    if (errno != 0)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"readdir failed errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
    }
      ...
      String *failureReport = nullptr)
  {
    ...
    if (std::filesystem::exists(artifactRoot, rootError) == false || std::filesystem::is_directory(artifactRoot, rootError) == false)
    {
      if (failureReport)
      {
        failureReport->snprintf<"artifact root {} is not a readable directory"_ctv>(artifactRootPath);
      }
      return false;
    }
    ...
    if (iteratorError)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to walk artifact root {}"_ctv>(artifactRootPath);
      }
      return false;
    }
    ...
    while (it != end)
    {
      ...
      if (totalEntries > maxEntries)
      {
        if (failureReport)
        {
          failureReport->snprintf<"artifact contains too many entries: {itoa} > {itoa}"_ctv>(
      ...
      if (iteratorError)
      {
        if (failureReport)
        {
          failureReport->snprintf<"failed to continue artifact walk {}"_ctv>(artifactRootPath);
        }
        return false;
      }
    }
  ...
  constexpr static auto containerCreatePendingMarkerRelativePath = ".prodigy-private/create.pending"_ctv;
  ...
  static bool writeToFileDescriptor(int fd, const String& payload, String *failureReport = nullptr)
  {
    ...
    while (written < payload.size())
    {
      ...
      if (bytesWritten < 0)
      {
        ...
        if (failureReport)
        {
          failureReport->snprintf<"write failed errno={}({})"_ctv>(errno, String(strerror(errno)));
        }
        return false;
      }
  ...
  static bool writeContainerCreatePendingMarker(const String& artifactRootPath, pid_t creatorPID, String *failureReport = nullptr)
  {
    ...
    if (artifactRootFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open artifact root {} without following symlinks errno={}({})"_ctv>(artifactRootPath, errno, String(strerror(errno)));
      }
      return false;
    }
    ...
    if (privateFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open artifact private metadata directory for pending-create marker errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      close(artifactRootFD);
      return false;
    }
    ...
    if (markerFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to create pending-create marker without following symlinks errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      close(privateFD);
      close(artifactRootFD);
      return false;
  ...
  static bool clearContainerCreatePendingMarker(const String& artifactRootPath, String *failureReport = nullptr)
  {
    ...
    if (privateFD < 0)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"failed to open artifact private metadata directory for pending-create marker cleanup errno={}({})"_ctv>(openErrno, String(strerror(openErrno)));
      }
      return false;
    }
    ...
    if (failureReport)
    {
      failureReport->snprintf<"failed to remove pending-create marker errno={}({})"_ctv>(unlinkErrno, String(strerror(unlinkErrno)));
    }
    return false;
  }
  ...
  static bool readContainerCreatePendingMarker(const String& artifactRootPath, bool& markerPresent, pid_t& creatorPID, String *failureReport = nullptr)
  {
    ...
    if (privateFD < 0)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"failed to open artifact private metadata directory while inspecting pending-create marker errno={}({})"_ctv>(openErrno, String(strerror(openErrno)));
      }
      return false;
    }
    ...
    if (markerFD < 0)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"failed to open pending-create marker without following symlinks errno={}({})"_ctv>(openErrno, String(strerror(openErrno)));
      }
      return false;
    }
    ...
    if (readOK == false)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to read pending-create marker: {}"_ctv>(readFailure);
      }
      return false;
    }
    ...
    if (tail == markerContents.c_str() || (tail != nullptr && *tail != '\0') || parsed <= 0 || parsed > std::numeric_limits<pid_t>::max())
    {
      creatorPID = -1;
      return true;
    }
      ...
      String *failureReport = nullptr)
  {
    ...
    if (statusError)
    {
      if (statusError == std::make_error_code(std::errc::no_such_file_or_directory) || statusError.value() == ENOENT)
      {
        return true;
      }
      ...
      String *failureReport = nullptr)
  {
    if (container == nullptr)
    {
      if (failureReport)
      {
        failureReport->assign("container missing for failed-container artifact retention"_ctv);
      }
      return false;
    }
    ...
    std::filesystem::path bundlePath =
        retentionRoot / applicationIDText.c_str() / containerNameText.c_str() / failureTimeText.c_str();
      ...
      String *failureReport = nullptr)
  {
    ...
    if (container->infop.si_pid <= 0)
    {
      if (failureReport)
      {
        failureReport->assign("failed-container artifact retention requires waitid siginfo"_ctv);
      }
      return false;
    }
  ...
  static bool cleanupRejectedOrphanedContainerArtifactsAtPath(const String& containersRootPath, String *failureReport = nullptr)
  {
    ...
    if (containersRootFD < 0)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"failed to open containers root {} without following symlinks errno={}({})"_ctv>(containersRootPath, errno, String(strerror(errno)));
      }
      return false;
    }
      ...
      String *failureReport = nullptr)
  {
    ...
    if (scratchFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open receive scratch {} without following symlinks errno={}({})"_ctv>(
            receiveScratchPath,
            errno,
            String(strerror(errno)));
      }
    ...
    if (entries.size() == 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"receive scratch {} produced no artifact"_ctv>(receiveScratchPath);
      }
      return false;
    }
    ...
    if (entries.size() != 1)
    {
      if (failureReport)
      {
        failureReport->snprintf<"receive scratch {} must contain exactly one top-level artifact entry, got {itoa}"_ctv>(
            receiveScratchPath,
    ...
    if (entries[0].size() == 0 || entries[0][0] == '.')
    {
      if (failureReport)
      {
        failureReport->snprintf<"receive scratch {} must not contain hidden top-level artifact entries"_ctv>(receiveScratchPath);
      }
      return false;
    }
  ...
  static bool ensureSizedBackingFile(const String& path, uint64_t targetBytes, String *failureReport = nullptr)
  {
    ...
    if (slash != size_t(-1))
    {
      ...
      if (parent.size() > 0 && createDirectoryTree(parent) == false)
      {
        if (failureReport)
        {
          failureReport->snprintf<"failed to create backing file parent {}"_ctv>(parent);
        }
        return false;
      }
    }
    ...
    if (fd < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open backing file {}: {}"_ctv>(path, String(strerror(errno)));
      }
      return false;
    }
    ...
    if (fstat(fd, &statbuf) != 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to stat backing file {}: {}"_ctv>(path, String(strerror(errno)));
      }
      close(fd);
      return false;
    }
    ...
    if (uint64_t(statbuf.st_size) < targetBytes)
    {
      if (ftruncate(fd, off_t(targetBytes)) != 0)
      {
        if (failureReport)
        {
          failureReport->snprintf<"failed to grow backing file {}: {}"_ctv>(path, String(strerror(errno)));
        }
        close(fd);
        return false;
      }
  ...
  {
    if (canonicalPath.equal("/dev/nvidiactl"_ctv) || canonicalPath.equal("/dev/nvidia-uvm"_ctv) || canonicalPath.equal("/dev/nvidia-uvm-tools"_ctv) || canonicalPath.equal("/dev/nvidia-modeset"_ctv))
    {
      return true;
    }

    return canonicalGPUDevicePathMatchesNumericSuffix(canonicalPath, "/dev/nvidia"_ctv) || canonicalGPUDevicePathMatchesNumericSuffix(canonicalPath, "/dev/dri/card"_ctv) || canonicalGPUDevicePathMatchesNumericSuffix(canonicalPath, "/dev/dri/renderD"_ctv);
  }
  ...
  constexpr static uint64_t secureContainerResolveFlags =
  ...
  static bool validateContainerRelativePathComponent(const String& component, const String& fullPath, String *failureReport = nullptr)
  {
    if (component.size() == 0 || component.equal("."_ctv) || component.equal(".."_ctv))
    {
      if (failureReport)
      {
        failureReport->snprintf<"container rootfs path {} contains an unsafe path component"_ctv>(fullPath);
      }
      return false;
    }
  ...
  static bool assignContainerDescriptorOwnership(int descriptor, uid_t userID, gid_t groupID, String *failureReport = nullptr)
  {
    if (descriptor < 0)
    {
      if (failureReport)
      {
        failureReport->assign("container descriptor is invalid"_ctv);
      }
      return false;
    }
  ...
  static bool openVerifiedContainerArtifactRoot(Container *container, int& artifactRootFD, String *failureReport = nullptr)
  {
    ...
    if (container == nullptr || container->artifactRootPath.size() == 0)
    {
      if (failureReport)
      {
        failureReport->assign("container artifact root path is missing"_ctv);
      }
      return false;
    }
...
private:
  ...
  static bool validateContainerArtifactShapeAt(int artifactRootFD, String *failureReport = nullptr)
  {
    ...
    for (const String& entryName : topLevelEntries)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"unexpected top-level artifact entry {}"_ctv>(entryName);
      }
      return false;
    }
    ...
    if (sawRootfs == false)
    {
      if (failureReport)
      {
        failureReport->assign("container artifact is missing required top-level rootfs directory"_ctv);
      }
      return false;
    }
    ...
    if (sawPrivateMetadataDirectory == false)
    {
      if (failureReport)
      {
        failureReport->assign("container artifact is missing required .prodigy-private directory"_ctv);
      }
      return false;
    }
    ...
    if (rootfsFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open artifact rootfs without following symlinks errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      return false;
    }
    ...
    if (fstat(rootfsFD, &rootfsStat) != 0 || S_ISDIR(rootfsStat.st_mode) == 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"artifact rootfs is not a directory errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      close(rootfsFD);
      return false;
    }
    ...
    if (privateFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open artifact private metadata directory without following symlinks errno={}({})"_ctv>(errno, String(strerror(errno)));
      }
      return false;
    }
    ...
    for (const String& privateEntryName : privateEntries)
    {
      ...
      if (failureReport)
      {
        failureReport->snprintf<"artifact private metadata directory contains unexpected entry {}"_ctv>(privateEntryName);
      }
      close(privateFD);
      return false;
    }
    ...
    else if (metadataIsRegular && uint64_t(metadataStat.st_size) > maxLaunchMetadataBytes)
    {
      if (failureReport)
      {
        failureReport->snprintf<"artifact launch.metadata exceeds maximum size: {} > {}"_ctv>(
      ...
      String *failureReport = nullptr)
  {
    if (expectedDigest.size() == 0)
    {
      if (failureReport)
      {
        failureReport->assign("container blob sha256 is missing from the plan"_ctv);
      }
      return false;
    }
    ...
    if (expectedBytes == 0)
    {
      if (failureReport)
      {
        failureReport->assign("container blob size is missing from the plan"_ctv);
      }
      return false;
    }
  ...
  static bool prepareContainerRootFSMountTargets(Container *container, int rootfd, String *failureReport = nullptr)
  {
    ...
    if (assignContainerDescriptorOwnership(devFD, userID, groupID, failureReport) == false || prepareContainerDeviceMountTarget(rootfd, devFD, "null"_ctv, userID, groupID, makedev(1, 3), failureReport) == false || prepareContainerDeviceMountTarget(rootfd, devFD, "zero"_ctv, userID, groupID, makedev(1, 5), failureReport) == false || prepareContainerDeviceMountTarget(rootfd, devFD, "random"_ctv, userID, groupID, makedev(1, 8), failureReport) == false || prepareContainerDeviceMountTarget(rootfd, devFD, "urandom"_ctv, userID, groupID, makedev(1, 9), failureReport) == false)
    {
      close(devFD);
      return false;
    }
  ...
  static bool bindMountHostDeviceIntoRootFS(const String& sourcePath, const String& containerRoot, String *failureReport = nullptr)
  {
    if (sourcePath.size() == 0 || containerRoot.size() == 0 || sourcePath[0] != '/')
    {
      if (failureReport)
      {
        failureReport->assign("host device mount source path must be absolute"_ctv);
      }
      return false;
    }
    ...
    if (pathStat(sourcePath, sourceStat) == false)
    {
      if (failureReport)
      {
        failureReport->snprintf<"host device source {} missing"_ctv>(sourcePath);
      }
      return false;
    }
    ...
    if (S_ISCHR(sourceStat.st_mode) == 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"host device source {} is not a character device"_ctv>(sourcePath);
      }
      return false;
    }
  ...
  static bool bindMountFileIntoRootFS(const String& sourcePath, const String& containerRoot, int containerRootFD, String *failureReport = nullptr)
  {
    ...
    if (isAllowlistedCanonicalGPUDevicePath(sourcePath) == false)
    {
      if (failureReport)
      {
        failureReport->snprintf<"gpu device source {} is not allowlisted"_ctv>(sourcePath);
      }
      return false;
    }
    ...
    if (pathStat(sourcePath, sourceStat) == false)
    {
      if (failureReport)
      {
        failureReport->snprintf<"gpu device source {} missing"_ctv>(sourcePath);
      }
      return false;
    }
    ...
    if (S_ISCHR(sourceStat.st_mode) == 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"gpu device source {} is not a character device"_ctv>(sourcePath);
      }
      return false;
    }
    ...
    for (const AssignedGPUDevice& gpu : container->plan.assignedGPUDevices)
    {
      ...
      if (vendorLower == "nvidia"_ctv)
      {
        ...
        if (mountedNvidiaMinor == false)
        {
          if (failureReport)
          {
            failureReport->snprintf<"failed to resolve nvidia gpu device for bus {}"_ctv>(gpu.busAddress);
          }
          return false;
        }
      }
      ...
      if (mountedAnyPerGPUDevice == false && vendorLower != "nvidia"_ctv)
      {
        if (failureReport)
        {
          failureReport->snprintf<"failed to resolve gpu device nodes for bus {}"_ctv>(gpu.busAddress);
        }
        return false;
      }
    }
...
public:
  ...
  static bool debugMountAssignedGPUDevicesInCurrentNamespace(Container *container, const String& containerRoot, String *failureReport = nullptr)
  {
    ...
    if (containerRootFD < 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to open debug container root {} without following symlinks errno={}({})"_ctv>(containerRoot, errno, String(strerror(errno)));
      }
      return false;
    }
  ...
  static bool debugPrepareBindMountFileTargetInRootFS(Container *container, const String& sourcePath, String *failureReport = nullptr)
  {
    if (sourcePath.size() == 0 || sourcePath[0] != '/')
    {
      if (failureReport)
      {
        failureReport->assign("bind mount target source path must be absolute"_ctv);
      }
      return false;
    }
  ...
  {
    return filesystemType.equal("ext4"_ctv) || filesystemType.equal("xfs"_ctv) || filesystemType.equal("btrfs"_ctv) || filesystemType.equal("f2fs"_ctv) || filesystemType.equal("zfs"_ctv) || filesystemType.equal("bcachefs"_ctv);
  }
  ...
  static bool mountLoopBackedStorage(Container *container, bool existingBackend, String *failureReport = nullptr)
  {
    ...
    if (createDirectoryTree(container->storageRootPath) == false)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to create storage root {}"_ctv>(container->storageRootPath);
      }
      return false;
    }
    ...
    for (Container::StorageLoopDevice& device : container->storageLoopDevices)
    {
      ...
      else if (Filesystem::fileExists(device.backingFilePath) == false)
      {
        if (failureReport)
        {
          failureReport->snprintf<"missing storage backing file {}"_ctv>(device.backingFilePath);
        }
        return false;
      }
    ...
    if (mount(container->storageLoopDevices[0].loopDevicePath.c_str(), container->storageRootPath.c_str(), "btrfs", MS_NOSUID | MS_NODEV, nullptr) != 0)
    {
      if (failureReport)
      {
        failureReport->snprintf<"failed to mount storage root {}: {}"_ctv>(container->storageRootPath, String(strerror(errno)));
      }
      return false;
    }
  ...
  static bool prepareContainerStorage(Container *container, String *failureReport = nullptr)
  {
    ...
    if (collectEligibleStorageDevicePlans(container->name, container->plan.config.storageMB, devicePlans) == false)
    {
      if (failureReport)
      {
        failureReport->assign("failed to collect storage devices"_ctv);
      }
      return false;
    }
  ...
  static bool resizeContainerStorage(Container *container, uint32_t targetStorageMB, String *failureReport = nullptr)
  {
    if (container == nullptr || targetStorageMB == 0)
    {
      if (failureReport)
      {
        failureReport->assign("target storage must be positive"_ctv);
      }
      return false;
    }
    ...
    if (currentStorageMB == 0 && targetStorageMB > 0)
    {
      if (failureReport)
      {
        failureReport->assign("runtime storage create from zero is unsupported"_ctv);
      }
      return false;
    }
    ...
    if (targetStorageMB > currentStorageMB)
    {
      ...
      if (prodigySplitContainerStorageAcrossDevices(targetStorageMB, uint32_t(container->storageLoopDevices.size()), newSizes) == false)
      {
        if (failureReport)
        {
          failureReport->assign("failed to split target storage across loop devices"_ctv);
        }
        return false;
      }
  ...
  static bool applyContainerCPUCoreTarget(Container *container, uint16_t targetCores, String *failureReport = nullptr)
  {
    if (targetCores == 0 || targetCores > 256)
    {
      if (failureReport)
      {
        failureReport->snprintf<"invalid target core count {itoa}"_ctv>(targetCores);
      }
      return false;
    }
    ...
    if (currentCores == 0 || currentCores > 256)
    {
      if (failureReport)
      {
        failureReport->snprintf<"invalid current core count {itoa}"_ctv>(currentCores);
      }
      return false;
    }
    ...
    if (applicationUsesSharedCPUs(container->plan.config))
    {
      ...
      if (openContainerSliceFD(container, sliceFD) == false)
      {
        if (failureReport)
        {
          failureReport->assign("failed to open container cgroup slice"_ctv);
        }
        return false;
      }
    ...
    if (targetCores > currentCores)
    {
      for (uint16_t index = currentCores; index < targetCores; index++)
      {
        ...
        if (core >= thisNeuron->lcoreCount)
        {
          if (failureReport)
          {
            failureReport->snprintf<"target core {itoa} exceeds machine core range"_ctv>(core);
          }
          return false;
        }
        ...
        if (thisNeuron->lcores[core] != 0)
        {
          if (failureReport)
          {
            failureReport->snprintf<"target core {itoa} unavailable"_ctv>(core);
          }
          return false;
        }
      }
  ...
  static bool applyContainerMemoryTarget(Container *container, uint32_t targetMemoryMB, String *failureReport = nullptr)
  {
    if (targetMemoryMB == 0)
    {
      if (failureReport)
      {
        failureReport->assign("target memory must be positive"_ctv);
      }
      return false;
    }
  ...
  static bool restrictToCapabilities(Container *container, String *failureReport = nullptr)
  {
    ...
    {
      if (container->plan.config.capabilities.contains(index))
      {
        continue;
      }
    ...
    for (int capability : container->plan.config.capabilities)
    {
      ...
      if (cap_set_flag(procCaps, CAP_PERMITTED, 1, &value, CAP_SET) != 0 || cap_set_flag(procCaps, CAP_EFFECTIVE, 1, &value, CAP_SET) != 0 || cap_set_flag(procCaps, CAP_INHERITABLE, 1, &value, CAP_SET) != 0)
      {
        if (failureReport)
        {
...
public:
  ...
  static bool startContainer(Container *container, bool isRestart = false, String *failureReport = nullptr)
  {
    ...
    if (container->pid == 0) // child
    {
      ...
      if (container->plan.useHostNetworkNamespace == false && enableContainerTCPFastOpen(container->plan.uuid, &fastOpenFailure) == false)
      {
  ...
  static void spinContainer(ContainerPlan plan, uint128_t replaceContainerUUID, const NeuronContainerMetricPolicy& metricPolicy) // copy this into here in case we suspend
  {
    ...
    if (startContainer(container, false, &failureReport) == false)
    {
      ...
      {
        failureReport.assign("startContainer failed"_ctv);
      }
      if (thisNeuron != nullptr && container->plan.whiteholes.empty() == false)
      {
  ...
  static void restartContainer(Container *container)
  {
    ...
    if (startContainer(container, true, &failureReport) == false)
    {
      if (failureReport.size() == 0)
      {
        failureReport.assign("restart startContainer failed"_ctv);
      }
      if (thisNeuron != nullptr && container->plan.whiteholes.empty() == false)
      {
        thisNeuron->closeWhiteholesForLocalContainer(container->plan.fragment);
