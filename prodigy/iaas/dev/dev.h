...
public:
  ...
  void getMachines(CoroutineStack *coro, const String& metro, bytell_hash_set<Machine *>& machines) override
  {
    ...
    if (gatewayPrivate4 == 0)
    {
      ...
      if (inet_pton(AF_INET, "10.0.0.1", &parsedGatewayIPv4) == 1)
      {
        gatewayPrivate4 = parsedGatewayIPv4.s_addr;
      }
    }
    ...
    for (String& private4String : privates)
    {
      ...
      if (private4Text == nullptr || private4Text[0] == '\0')
      {
        continue;
      }

      struct in_addr parsedIPv4 = {};
      if (inet_pton(AF_INET, private4Text, &parsedIPv4) != 1)
      {
        continue;
      }
      uint32_t private4 = parsedIPv4.s_addr;
...
public:
  ...
  void setLocalContainerPrefixes(const Vector<IPPrefix>& prefixes) override
  {
    for (const IPPrefix& prefix : prefixes)
    {
      ...
      int rc = posix_spawnp(&pid, "radvd", &fa, nullptr, argv, environ);
      (void)rc;
