...
public:
  ...
  int start(const String& path, Sink sink, void *opaque)
  {
    ...
    ev.data.u32 = 1;
    if (epoll_ctl(epfd, EPOLL_CTL_ADD, inofd, &ev) < 0)
    {
      return -1;
    }
    return epfd;
...
static inline String make_crash_bundle(const ContainerLogPaths& p, int64_t crashTimeMs)
{
  ...
  if (zstd_compress_file(p.stdoutPath, soGz) != 0)
  {
    return String();
  }
  if (zstd_compress_file(p.stderrPath, seGz) != 0)
  {
    return String();
  }
  return outDir;
