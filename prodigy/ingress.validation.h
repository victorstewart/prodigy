...
constexpr static uint32_t maxMetricPairsPerFrame = 1024;
constexpr static uint32_t maxContainerMessagePayloadBytes = 1U << 20;
...
static bool validateMothershipPayload(uint16_t rawTopic, uint8_t *args, uint8_t *terminal)
{
  ...
  switch (MothershipTopic(rawTopic))
  {
    ...
    case MothershipTopic::spinApplication:
      {
        uint16_t applicationID = 0;
        if (extractFixed(cursor, terminal, applicationID) == false)
        {
          return false;
        }
    ...
    case BrainTopic::replicateMetricsAppend:
      {
        ...
        if (extractFixed(cursor, terminal, sampleTimeMs) == false)
        {
          return false;
        }
        if (extractFixed(cursor, terminal, metricKey) == false)
    ...
    case BrainTopic::registration:
      {
        ...
        if (extractFixed(cursor, terminal, uuid) == false)
        {
          return false;
        }
        if (extractFixed(cursor, terminal, boottimens) == false)
        {
          return false;
        }
        if (extractFixed(cursor, terminal, version) == false)
        {
          return false;
        }
        if (extractFixed(cursor, terminal, existingMasterUUID) == false)
        {
          return false;
        }
        if (consumeVariable(cursor, terminal) == false)
    ...
    case BrainTopic::masterMissing:
      {
        ...
        if (extractFixed(cursor, terminal, isMissing) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case BrainTopic::peerHeartbeat:
      {
        ...
        if (extractFixed(cursor, terminal, isResponse) == false)
        {
          return false;
        }
        if (extractFixed(cursor, terminal, heartbeatNonce) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case BrainTopic::transitionToNewBundle:
      {
        ...
        uint8_t marker = 0;
        if (extractFixed(cursor, terminal, marker) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case BrainTopic::relinquishMasterStatus:
      {
        ...
        if (extractFixed(cursor, terminal, designatedMasterUUID) == false)
        {
          return false;
        }
        return (cursor == terminal);
...
static bool validateNeuronPayloadForBrain(uint16_t rawTopic, uint8_t *args, uint8_t *terminal)
{
  ...
  switch (NeuronTopic(rawTopic))
  {
    case NeuronTopic::registration:
      {
        ...
        if (extractFixed(cursor, terminal, bootTimeMs) == false)
        {
          return false;
        }
        if (consumeVariable(cursor, terminal) == false)
        ...
        if (extractFixed(cursor, terminal, haveData) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case NeuronTopic::stateUpload:
      {
        uint32_t fragment = 0;
        if (extractFixed(cursor, terminal, fragment) == false)
        {
          return false;
        }
    ...
    case NeuronTopic::containerFailed:
      {
        ...
        if (extractFixed(cursor, terminal, approxTimeMs) == false)
        {
          return false;
        }
        if (extractFixed(cursor, terminal, signal) == false)
        {
          return false;
        }
        if (consumeVariable(cursor, terminal) == false)
        ...
        if (extractFixed(cursor, terminal, restarted) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case NeuronTopic::registration:
      {
        ...
        if (extractFixed(cursor, terminal, requiresState) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case NeuronTopic::spinContainer:
      {
        ...
        if (extractFixed(cursor, terminal, replaceContainerUUID) == false)
        {
          return false;
        }
        if (consumeVariable(cursor, terminal) == false)
    ...
    case NeuronTopic::adjustContainerResources:
      {
        ...
        if (extractFixed(cursor, terminal, nLogicalCores) == false)
        {
          return false;
        }
        if (extractFixed(cursor, terminal, memoryMB) == false)
        {
          return false;
        }
        if (extractFixed(cursor, terminal, storageMB) == false)
        {
          return false;
        }
        ...
        if (extractFixed(cursor, terminal, isDownscale) == false)
        {
          return false;
        }
        ...
        if (extractFixed(cursor, terminal, graceSeconds) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case NeuronTopic::changeContainerLifetime:
      {
        ...
        if (extractFixed(cursor, terminal, lifetime) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case NeuronTopic::openSwitchboardWhiteholes:
      {
        ...
        while (cursor < terminal)
        {
          ...
          if (extractFixed(cursor, terminal, sourcePort) == false)
          {
            return false;
          }
          if (consumeBytes(cursor, terminal, 16) == false)
          {
            return false;
          }
          if (extractFixed(cursor, terminal, isIPv6) == false)
          {
            return false;
          }
          if (extractFixed(cursor, terminal, transport) == false)
          {
            return false;
          }
          if (extractFixed(cursor, terminal, bindingNonce) == false)
          {
            return false;
          }
        }
...
static bool validateContainerPayloadForNeuron(uint16_t rawTopic, uint8_t *args, uint8_t *terminal)
{
  ...
  switch (ContainerTopic(rawTopic))
  {
    ...
    case ContainerTopic::resourceDeltaAck:
      {
        bool accepted = false;
        if (extractFixed(cursor, terminal, accepted) == false)
        {
          return false;
        }
        return (cursor == terminal);
    ...
    case ContainerTopic::datacenterUniqueTag:
      {
        ...
        if (extractFixed(cursor, terminal, datacenterUniqueTag) == false)
        {
          return false;
        }
        return (cursor == terminal);
