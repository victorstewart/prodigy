...
{
  if (state == NULL || state->callback_result != PRODIGY_RESULT_OK || result == PRODIGY_RESULT_OK)
  {
    return;
  }
...
static prodigy_result mesh_pingpong_open_listener(mesh_pingpong_state *state)
{
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &one, (socklen_t)sizeof(one)) != 0 || setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &one, (socklen_t)sizeof(one)) != 0 || bind(fd, (const struct sockaddr *)&local, (socklen_t)sizeof(local)) != 0 || listen(fd, 4) != 0)
  {
    close(fd);
    return PRODIGY_RESULT_IO;
  }
    ...
    const prodigy_subscription_pairing *pairing)
{
  ...
  if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &one, (socklen_t)sizeof(one)) != 0 || bind(fd, (const struct sockaddr *)&local, (socklen_t)sizeof(local)) != 0)
  {
    close(fd);
    return PRODIGY_RESULT_IO;
  }

  if (connect(fd, (const struct sockaddr *)&remote, (socklen_t)sizeof(remote)) != 0 && errno != EINPROGRESS)
  {
    close(fd);
    return PRODIGY_RESULT_IO;
  }
...
static int run(int argc, char **argv)
{
  ...
  while ((result = prodigy_io_uring_reactor_next(state.reactor, &event)) == PRODIGY_RESULT_OK)
  {
    ...
    if (event.kind == PRODIGY_IO_URING_REACTOR_EVENT_NEURON && (event.neuron_event == PRODIGY_IO_URING_NEURON_EVENT_SHUTDOWN || event.neuron_event == PRODIGY_IO_URING_NEURON_EVENT_CLOSED))
    {
      result = PRODIGY_RESULT_OK;
      break;
    }
