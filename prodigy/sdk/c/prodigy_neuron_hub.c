...
static prodigy_result prodigy_read_u64(prodigy_reader *reader, uint64_t *value)
{
  ...
  *value = (uint64_t)bytes[0] | ((uint64_t)bytes[1] << 8) | ((uint64_t)bytes[2] << 16) | ((uint64_t)bytes[3] << 24) | ((uint64_t)bytes[4] << 32) | ((uint64_t)bytes[5] << 40) | ((uint64_t)bytes[6] << 48) | ((uint64_t)bytes[7] << 56);
  return PRODIGY_RESULT_OK;
}
    ...
    prodigy_message_frame *frame)
{
  ...
  frame_size = (uint32_t)data[0] | ((uint32_t)data[1] << 8) | ((uint32_t)data[2] << 16) | ((uint32_t)data[3] << 24);
    ...
    prodigy_message_frame *frame)
{
  ...
  frame_size = (uint32_t)decoder->data[0] | ((uint32_t)decoder->data[1] << 8) | ((uint32_t)decoder->data[2] << 16) | ((uint32_t)decoder->data[3] << 24);
...
prodigy_result prodigy_neuron_hub_run_once(prodigy_neuron_hub *hub)
{
  ...
  frame_size = (uint32_t)header[0] | ((uint32_t)header[1] << 8) | ((uint32_t)header[2] << 16) | ((uint32_t)header[3] << 24);
  padding = header[6];
