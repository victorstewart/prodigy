    ...
    String *failure = nullptr)
{
  ...
  if (session == nullptr)
  {
    if (failure)
    {
      failure->assign("failed to initialize ssh session"_ctv);
    }
    ::close(fd);
    fd = -1;
    return false;
  ...
  auto awaitChannelOp = [&](auto&& operation, const char *phase) -> bool {
    ...
    while (true)
    {
      ...
      if (rc != LIBSSH2_ERROR_EAGAIN)
      {
        if (failure)
        {
          failure->snprintf<"failed to {} remote command {}"_ctv>(phaseText, command);
        }
        return false;
      }
      ...
      if (nowMs >= deadlineMs)
      {
        if (failure)
        {
          failure->snprintf<"timed out while trying to {} remote command {}"_ctv>(phaseText, command);
        }
        return false;
      }
      ...
      if (prodigyWaitForBlockingSSHSessionIO(session, fd, remainingMs) == false)
      {
        if (failure)
        {
          failure->snprintf<"timed out while waiting for ssh io to {} remote command {}"_ctv>(phaseText, command);
        }
        return false;
      }
    }
...
public:
  ...
  {
    if (totalLogicalCores != other.totalLogicalCores || totalMemoryMB != other.totalMemoryMB || totalStorageMB != other.totalStorageMB || peerAddresses.size() != other.peerAddresses.size())
    {
      return false;
    }
...
static inline bool prodigyParseRemoteMachineResources(const String& output, ProdigyRemoteMachineResources& resources, String *failure = nullptr)
{
  ...
  if (index < 3)
  {
    if (failure)
    {
      failure->assign("remote resource probe returned fewer than 3 numeric lines"_ctv);
    }
    return false;
  }

  if (prodigyParseUnsignedDecimalString(lines[0], resources.totalLogicalCores) == false || prodigyParseUnsignedDecimalString(lines[1], resources.totalMemoryMB) == false || prodigyParseUnsignedDecimalString(lines[2], resources.totalStorageMB) == false)
  {
    if (failure)
    {
      failure->assign("remote resource probe returned invalid numeric output"_ctv);
    }
    return false;
  }
  ...
  if (resources.totalLogicalCores == 0 || resources.totalMemoryMB == 0 || resources.totalStorageMB == 0)
  {
    if (failure)
    {
      failure->assign("remote resource probe returned zero resources"_ctv);
    }
    return false;
  }
...
static inline bool prodigyProbeRemoteMachineResources(const ClusterMachine& clusterMachine, ProdigyRemoteMachineResources& resources, String *failure = nullptr)
{
  ...
  if (prodigyResolveClusterMachineSSHAddress(clusterMachine, sshAddress) == false)
  {
    if (failure)
    {
      failure->snprintf<"machine '{}' has no ssh address"_ctv>(label);
    }
    return false;
  }
  ...
  if (clusterMachine.ssh.user.size() == 0)
  {
    if (failure)
    {
      failure->snprintf<"machine '{}' has no ssh user"_ctv>(label);
    }
    return false;
  }
  ...
  if (clusterMachine.ssh.hostPublicKeyOpenSSH.size() == 0)
  {
    if (failure)
    {
      failure->snprintf<"machine '{}' has no ssh.hostPublicKeyOpenSSH"_ctv>(label);
    }
    return false;
  }
  ...
  if (clusterMachine.ssh.privateKeyPath.size() == 0)
  {
    if (failure)
    {
      failure->snprintf<"machine '{}' has no sshPrivateKeyPath"_ctv>(label);
    }
    return false;
  }
  ...
  String output;
  bool success = prodigyRunBlockingSSHCommand(session, fd, command, &output, failure) && prodigyParseRemoteMachineResources(output, resources, failure);
  prodigyCloseBlockingSSHSession(session, fd);
  return success;
}
...
static inline bool prodigyUploadLocalFileToSSHSession(LIBSSH2_SESSION *session, int fd, const String& localPath, const String& remotePath, long permissions, String *failure = nullptr, int timeoutMs = 120'000)
{
  ...
  if (sftp == nullptr)
  {
    ...
    if (failure)
    {
      failure->assign("failed to initialize ssh sftp session"_ctv);
    }
    return false;
  }
  ...
  if (handle == nullptr)
  {
    ...
    if (failure)
    {
      failure->snprintf<"failed to open remote file {}"_ctv>(remotePath);
    }
    return false;
  }
    ...
    String *failure = nullptr)
{
  ...
  if (localUUID == 0)
  {
    if (failure)
    {
      failure->assign("failed to resolve bootstrap uuid for machine"_ctv);
    }
    return false;
  }
  ...
  if (clusterTLS->rootCertPem.size() == 0 || clusterTLS->rootKeyPem.size() == 0)
  {
    ...
    if (failure)
    {
      failure->assign("transport tls issuer unavailable for multi-node bootstrap"_ctv);
    }
    return false;
  }
...
static inline bool prodigyBuildRemoteBootstrapPlan(const ClusterMachine& clusterMachine, const AddMachines& request, const ClusterTopology& topology, const ProdigyRuntimeEnvironmentConfig& runtimeEnvironment, ProdigyRemoteBootstrapPlan& plan, String *failure = nullptr)
{
  ...
  if (request.controlSocketPath.size() == 0)
  {
    if (failure)
    {
      failure->assign("addMachines controlSocketPath required"_ctv);
    }
    return false;
  }
  ...
  if (request.clusterUUID == 0)
  {
    if (failure)
    {
      failure->assign("addMachines clusterUUID required"_ctv);
    }
    return false;
  }
  ...
  if (prodigyMachineCpuArchitectureSupportedTarget(request.architecture) == false)
  {
    if (failure)
    {
      failure->assign("addMachines architecture required"_ctv);
    }
    return false;
  }
  ...
  if (request.remoteProdigyPath.size() == 0)
  {
    if (failure)
    {
      failure->assign("addMachines remoteProdigyPath required"_ctv);
    }
    return false;
  }
  ...
  if (clusterMachine.ssh.privateKeyPath.size() == 0 && prodigyBootstrapSSHKeyPackageConfigured(request.bootstrapSshKeyPackage) == false)
  {
    if (failure)
    {
      failure->snprintf<"machine '{}' has no sshPrivateKeyPath"_ctv>(label);
  ...
  if (prodigyBootstrapSSHKeyPackageConfigured(request.bootstrapSshKeyPackage))
  {
    ...
    if (plan.bootstrapSSHDirectory.size() == 0)
    {
      if (failure)
      {
        failure->assign("bootstrapSshPrivateKeyPath must include a directory"_ctv);
      }
      return false;
    }
  }
...
static inline bool prodigyBuildPreparedRemoteBootstrapPayload(ProdigyPreparedRemoteBootstrapPlan& prepared, String *failure = nullptr)
{
  ...
  {
    String stagedPath = {};
    if (prodigyBuildRemoteBootstrapStagePath(stagingRoot, prepared.plan.remoteBootstrapSSHPrivateKeyPath, stagedPath, failure) == false || prodigyWriteLocalFile(stagedPath, prepared.plan.bootstrapSshKeyPackage.privateKeyOpenSSH, 0600, failure) == false)
    {
      return failBuild();
    }

    if (prodigyBuildRemoteBootstrapStagePath(stagingRoot, prepared.plan.remoteBootstrapSSHPublicKeyPath, stagedPath, failure) == false || prodigyWriteLocalFile(stagedPath, prepared.plan.bootstrapSshKeyPackage.publicKeyOpenSSH, 0644, failure) == false)
    {
      return failBuild();
    }
  }
  ...
  String stagedPath = {};
  if (prodigyBuildRemoteBootstrapStagePath(stagingRoot, prepared.plan.installPaths.bundleSHA256TempPath, stagedPath, failure) == false || prodigyWriteLocalFile(stagedPath, prepared.bundleSHA256Content, 0644, failure) == false)
  {
    return failBuild();
  }

  if (prodigyBuildRemoteBootstrapStagePath(stagingRoot, prepared.plan.remoteBootJSONPath, stagedPath, failure) == false || prodigyWriteLocalFile(stagedPath, prepared.plan.bootJSON, 0600, failure) == false)
  {
    return failBuild();
  }

  if (prodigyBuildRemoteBootstrapStagePath(stagingRoot, prepared.plan.remoteTransportTLSJSONPath, stagedPath, failure) == false || prodigyWriteLocalFile(stagedPath, prepared.plan.transportTLSJSON, 0600, failure) == false)
  {
    return failBuild();
  }

  if (prodigyBuildRemoteBootstrapStagePath(stagingRoot, prepared.plan.remoteUnitTempPath, stagedPath, failure) == false || prodigyWriteLocalFile(stagedPath, prepared.plan.systemdUnit, 0644, failure) == false)
  {
    return failBuild();
  }
    ...
    String *failure = nullptr)
{
  ...
  prepared.plan = plan;
  if (bundleApprovalCache != nullptr && bundleApprovalCache->localBundlePath.size() > 0 && bundleApprovalCache->localBundlePath.equals(plan.localBundlePath) && bundleApprovalCache->bundleSHA256.size() > 0)
  {
