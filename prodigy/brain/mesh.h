...
private:
  ...
  struct ServicePairing {
    ...
    ServicePairing(MeshNode *_advertiser, MeshNode *_subscriber, uint64_t _service) noexcept
  ...
  struct AnyCmp {
    ...
    bool operator()(const AnyEntry& a, const AnyEntry& b) const
    {
      ...
      {
        return a.capacity < b.capacity; // max-heap by capacity
      }
  ...
  struct SomeEntry {
    ...
    SomeEntry(uint32_t c, MeshNode *n, uint32_t e)
        : count(c),
          node(n),
          epoch(e)
    {}
  ...
  struct SomeCmp {
    ...
    bool operator()(const SomeEntry& a, const SomeEntry& b) const
    {
      ...
      if (a.epoch != b.epoch)
      {
        return a.epoch < b.epoch; // prefer latest epoch
      }
      return a.node > b.node;
    }
  };
  ...
  void createPairing(MeshNode *advertiser, MeshNode *subscriber, uint64_t service, bool notifyAdvertiser, bool notifySubscriber)
  {
    ...
    if (auto existing = pairingSecrets.find(ServicePairing {advertiser, subscriber, service}); existing != pairingSecrets.end())
    {
      ...
      if (hadAdvertiserEdge == false)
      {
        advertiser->advertisingTo.insert(service, subscriber);
      }
      if (hadSubscriberEdge == false)
      {
        subscriber->subscribedTo.insert(service, advertiser);
      }
      if (shouldNotifyAdvertiser && hadAdvertiserEdge == false)
      {
        advertiser->advertisementPairing(secret, subscriber->pairingAddress(), service, subscriber->applicationID, true);
      }
      if (shouldNotifySubscriber && hadSubscriberEdge == false)
      {
        subscriber->subscriptionPairing(secret, advertiser->pairingAddress(), service, advertiserPort, advertiser->applicationID, true);
      }
      return;
    }
  ...
  void destroyPairing(const char *reason, MeshNode *advertiser, MeshNode *subscriber, uint64_t service, bool notifyAdvertiser, bool notifySubscriber, bool eraseFromAdvertiser, bool eraseFromSubscriber)
  {
    ...
    if (eraseFromAdvertiser)
    {
      advertiser->advertisingTo.eraseEntry(service, subscriber);
    }
    // secret(16) service(8) activate(1)
    if (notifyAdvertiser)
    {
      advertiser->advertisementPairing(oldSecret, subscriber->pairingAddress(), service, subscriber->applicationID, false);
    }

    if (eraseFromSubscriber)
    {
      subscriber->subscribedTo.eraseEntry(service, advertiser);
    }
    // secret(16) address(16) service(8) port(2) activate(1)
    if (notifySubscriber)
    {
      subscriber->subscriptionPairing(oldSecret, advertiser->pairingAddress(), service, advertiserPort, advertiser->applicationID, false);
    }
  ...
  struct BootingPairing {
    ...
    BootingPairing(MeshNode *_advertiser, MeshNode *_subscriber, uint64_t _service)
        : advertiser(_advertiser),
          subscriber(_subscriber),
          service(_service)
    {}
      ...
      case SubscriptionNature::exclusiveSome:
        {
          ...
          if (someHeapEpochs.contains(service) == false)
          {
            someHeapEpochs.insert_or_assign(service, 0u);
          }
          pushSomeEntry(service, subscriber, cnt);
          enforceSomeHeapBudget(service);
          uint64_t matchedService = 0;
      ...
      case SubscriptionNature::exclusiveSome: // evenly distribute subscribers over advertisers
        {
          ...
          if (counts.contains(subscriber) == false)
          {
            counts.insert_or_assign(subscriber, subscriber->subscribedTo.countEntriesFor(service));
          }
          pushSomeEntry(service, subscriber, counts[subscriber]);
          enforceSomeHeapBudget(service);
