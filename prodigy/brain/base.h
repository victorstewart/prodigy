...
public:
  ...
  constexpr static int64_t metricRetentionMs = prodigyBrainMetricRetentionMs; // 6h
  constexpr static int64_t metricTrimMinIntervalMs = prodigyBrainMetricTrimMinIntervalMs;
  constexpr static int64_t metricPersistMinIntervalMs = prodigyBrainMetricPersistMinIntervalMs;
  constexpr static int64_t autoscaleIntervalMs = prodigyBrainAutoscaleIntervalMs; // 1 minute
...
public:
  ...
  constexpr static uint64_t brainPeerReplicationBufferedBytesLimit = 512_MB;
  constexpr static uint32_t brainPeerLargePayloadKeepaliveSeconds = 120u;
  constexpr static uint64_t brainPeerReplicationFrameHeadroomBytes = 256_KB;
  ...
  static int brainPeerLargePayloadKeepaliveIdleSeconds(void)
  {
    ...
    int tcpKeepIdleVal = int(brainPeerLargePayloadKeepaliveSeconds) - (keepaliveProbeCount + 1) * keepaliveProbeIntervalSeconds;
    if (tcpKeepIdleVal < 1)
    {
      tcpKeepIdleVal = 1;
    }
  ...
  void queueBrainSystemContainerArtifactReplication(const String& sha256, uint64_t bytes, const String& blob)
  {
    for (BrainView *brain : brains)
    {
      if (brain == nullptr)
      ...
      (void)queueBrainSystemContainerArtifactReplicationToPeer(brain, sha256, bytes, blob);
    }
  }
