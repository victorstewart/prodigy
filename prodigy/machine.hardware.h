...
static inline bool prodigyDecodeMachineHardwareCommandStatus(int rawStatus, int savedErrno, int *exitStatus = nullptr, String *failure = nullptr)
{
  ...
  if (rawStatus < 0)
  {
    if (failure)
    {
      failure->snprintf<"failed to reap local command: {}"_ctv>(String(strerror(savedErrno)));
    }
    return false;
  }
  ...
  if (WIFEXITED(rawStatus))
  {
    ...
    if (failure)
    {
      failure->snprintf<"local command exited with status {itoa}"_ctv>(uint32_t(code));
    }
    return false;
  }
  ...
  if (WIFSIGNALED(rawStatus))
  {
    ...
    if (failure)
    {
      failure->snprintf<"local command terminated by signal {itoa}"_ctv>(uint32_t(signalNumber));
    }
    return false;
  }
  ...
  if (failure)
  {
    failure->snprintf<"local command ended with unexpected wait status {itoa}"_ctv>(uint32_t(rawStatus));
  }
  return false;
}
...
static inline bool prodigyRunBlockingLocalCommandCaptureStdout(const String& command, String& output, int *exitStatus = nullptr, String *failure = nullptr)
{
  ...
  if (access(shellPath, X_OK) != 0)
  {
    if (failure)
    {
      failure->assign("failed to launch local command: missing shell"_ctv);
    }
    return false;
  }
  ...
  if (pipe2(pipeFDs, O_CLOEXEC) != 0)
  {
    if (failure)
    {
      failure->assign("failed to launch local command"_ctv);
    }
    return false;
  }
  ...
  if (spawnSetupRC == 0)
  {
    spawnSetupRC = posix_spawn_file_actions_adddup2(&actions, pipeFDs[1], STDOUT_FILENO);
  }
  if (spawnSetupRC == 0)
  {
    spawnSetupRC = posix_spawn_file_actions_addclose(&actions, pipeFDs[0]);
  }
#ifdef __GLIBC__
  if (spawnSetupRC == 0)
  {
    spawnSetupRC = posix_spawn_file_actions_addclosefrom_np(&actions, 3);
  }
#endif
  if (spawnSetupRC == 0)
  {
    spawnSetupRC = posix_spawnattr_init(&attrs);
  }
  ...
  if (spawnSetupRC == 0)
  {
    spawnSetupRC = posix_spawnattr_setsigmask(&attrs, &emptyMask);
  }
  if (spawnSetupRC == 0)
  {
    spawnSetupRC = posix_spawnattr_setsigdefault(&attrs, &defaultSignals);
  }
  if (spawnSetupRC == 0)
  {
    spawnSetupRC = posix_spawnattr_setflags(&attrs, POSIX_SPAWN_SETSIGMASK | POSIX_SPAWN_SETSIGDEF);
  }
  ...
  if (spawnRC != 0)
  {
    ...
    if (failure)
    {
      failure->snprintf<"failed to launch local command: {}"_ctv>(String(strerror(spawnRC)));
    }
    return false;
  }
...
static inline bool prodigyResolveOfficialSpeedtestCommand(String& toolCommand)
{
  ...
  if (prodigyRunBlockingLocalCommandCaptureStdout(versionCommand, versionOutput) == false || prodigyStringContains(versionOutput, "Speedtest by Ookla") == false)
  {
    return false;
  }
...
static inline bool prodigyParseLscpuTopologyCSV(const String& csv, ProdigyCpuInventorySnapshot& snapshot)
{
  ...
  while (start < csv.size())
  {
    ...
    if (end > start && csv[start] != '#')
    {
      ...
      {
        uint32_t cpuID = 0;
        bool online = (columns[1].equal("Y"_ctv) || columns[1].equal("y"_ctv) || columns[1].equal("1"_ctv) || columns[1].equal("yes"_ctv) || columns[1].equal("YES"_ctv) || columns[1].equal("true"_ctv) || columns[1].equal("TRUE"_ctv));
...
static inline bool prodigyParseSysbenchCpuOutput(const String& output, uint64_t& score, String *failure = nullptr)
{
  ...
  if (text == nullptr)
  {
    if (failure)
    {
      failure->assign("sysbench output empty"_ctv);
    }
    return false;
  }
  ...
  if (failure)
  {
    failure->assign("sysbench output malformed"_ctv);
  }
  return false;
}
    ...
    String *failure = nullptr)
{
  ...
  if (prodigyResolveMachineHardwareToolCommand("sysbench"_ctv, sysbenchCommand) == false)
  {
    if (failure)
    {
      failure->assign("sysbench unavailable"_ctv);
    }
    return false;
  }
...
static inline void prodigyCollectMemoryHardwareProfile(MachineMemoryHardwareProfile& memory, const MachineCpuHardwareProfile& cpu, const ProdigyMachineHardwareCollectorOptions& options)
{
  ...
  String bwMemCommand = {};
  if (prodigyResolveMachineHardwareToolCommand("lat_mem_rd"_ctv, latMemCommand) == false || prodigyResolveMachineHardwareToolCommand("bw_mem"_ctv, bwMemCommand) == false)
  {
    return;
  }
...
static inline bool prodigyParseLsblkPairsLine(const String& line, ProdigyLsblkRow& row)
{
  ...
  while (i < line.size())
  {
    ...
    else if (key.equal("MODEL"_ctv))
    {
      row.model = value;
    }
    else if (key.equal("SERIAL"_ctv))
    {
      row.serial = value;
    }
    ...
    else if (key.equal("TRAN"_ctv))
    {
      row.transport = value;
    }
    else if (key.equal("MOUNTPOINT"_ctv))
    {
      row.mountPath = value;
    }
    else if (key.equal("PKNAME"_ctv))
    {
      row.parentName = value;
    }
    else if (key.equal("ROTA"_ctv))
    {
      row.rotational = value.equal("1"_ctv);
    }
...
static inline bool prodigyParseLsblkJSON(const String& json, Vector<ProdigyLsblkRow>& rows)
{
  ...
  std::function<void(simdjson::dom::element, const String&)> visitNode = [&](simdjson::dom::element node, const String& parentName) -> void {
    ...
    if (node["name"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
      row.name.assign(value.get_c_str());
    }
    if (node["kname"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
      row.kernelName.assign(value.get_c_str());
    }
    if (node["path"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
      row.path.assign(value.get_c_str());
    }
    if (node["model"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
      row.model.assign(value.get_c_str());
    }
    if (node["serial"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
      row.serial.assign(value.get_c_str());
    }
    if (node["wwn"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
      row.wwn.assign(value.get_c_str());
    }
    if (node["type"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
      row.type.assign(value.get_c_str());
    }
    if (node["tran"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
      row.transport.assign(value.get_c_str());
    }
    uint64_t numericValue = 0;
    if (node["rota"].get_uint64().get(numericValue) == simdjson::SUCCESS)
    {
      row.rotational = (numericValue != 0);
    }
    (void)node["size"].get_uint64().get(row.sizeBytes);
    if (node["log-sec"].get_uint64().get(numericValue) == simdjson::SUCCESS)
    {
      row.logicalSectorBytes = uint32_t(numericValue);
    }
    if (node["phy-sec"].get_uint64().get(numericValue) == simdjson::SUCCESS)
    {
      row.physicalSectorBytes = uint32_t(numericValue);
    }
...
static inline void prodigyCollectPciDescriptions(Vector<ProdigyPciDescription>& descriptors, Vector<MachineToolCapture> *captures = nullptr)
{
  ...
  String output = {};
  if ((captures && prodigyRunRecordedLocalCommand("lspci"_ctv, "inventory"_ctv, command, *captures, output) == false) || (captures == nullptr && prodigyRunBlockingLocalCommandCaptureStdout(command, output) == false))
  {
    return;
  }
    ...
    IPAddress *sourceAddress = nullptr)
{
  ...
  if (doc["ping"].get(pingNode) != simdjson::SUCCESS || doc["download"].get(downloadNode) != simdjson::SUCCESS || doc["upload"].get(uploadNode) != simdjson::SUCCESS)
  {
    return false;
  }
  ...
  if (parseDoubleValue(pingNode, "latency", pingLatency) == false || parseDoubleValue(downloadNode, "bandwidth", downloadBytesPerSecond) == false || parseDoubleValue(uploadNode, "bandwidth", uploadBytesPerSecond) == false)
  {
    return false;
  }
  ...
  if (doc["interface"].get(interfaceNode) == simdjson::SUCCESS)
  {
    ...
    if (interfaceName && interfaceNode["name"].get(value) == simdjson::SUCCESS && value.type() == simdjson::dom::element_type::STRING)
    {
...
static inline void prodigyTagInternetReachableNicSubnets(MachineNetworkHardwareProfile& network)
{
  ...
  auto markMatchingSubnet = [&](bool requireInterfaceName) -> bool {
    for (MachineNicHardwareProfile& nic : network.nics)
    {
      if (requireInterfaceName && network.internet.interfaceName.size() > 0 && nic.name.equals(network.internet.interfaceName) == false)
      {
        continue;
      }
...
static inline void prodigyCollectMachineHardwareProfile(MachineHardwareProfile& hardware, const ProdigyMachineHardwareCollectorOptions& options = {})
{
  ...
  hardware.inventoryComplete =
      (hardware.cpu.logicalCores > 0 && hardware.memory.totalMB > 0 && (haveDisks || hardware.disks.size() > 0) && haveNetworkAddressing);
