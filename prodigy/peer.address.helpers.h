...
static inline void prodigyCollectLocalPeerAddressCandidates(const String& preferredInterface, const IPAddress& fallbackPrivate4, Vector<ClusterMachinePeerAddress>& candidates)
{
  ...
  auto collect = [&](bool preferredOnly) -> void {
    ...
    for (const struct ifaddrs *ifa = interfaces; ifa != nullptr; ifa = ifa->ifa_next)
    {
      ...
      if (preferredOnly && preferredInterface.size() > 0 && (std::strncmp(reinterpret_cast<const char *>(preferredInterface.data()), ifa->ifa_name, preferredInterface.size()) != 0 || ifa->ifa_name[preferredInterface.size()] != '\0'))
      {
        continue;
      }
    ...
    String *sourceAddressText = nullptr)
{
  ...
  for (const ClusterMachinePeerAddress& localCandidate : localCandidates)
  {
    ...
    if (haveRemotePrivateSubnet && prodigyClusterMachinePeerAddressSubnetKey(localCandidate, localPrivateSubnet, true) && localPrivateSubnet.equals(remotePrivateSubnet))
    {
      score += 100;
    }

    String localSubnet = {};
    if (haveRemoteSubnet && prodigyClusterMachinePeerAddressSubnetKey(localCandidate, localSubnet, false) && localSubnet.equals(remoteSubnet))
    {
      score += 40;
    }
