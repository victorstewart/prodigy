...
__attribute__((__always_inline__)) static inline struct packet_frame *getFrame(struct packet *pkt)
{
  switch (pkt->nFrames)
  {
    case 0:
...
__attribute__((__always_inline__)) static inline void logTcpFrame(struct tcphdr *tcp, struct packet *pkt, void *data_end)
{
...
__attribute__((__always_inline__)) static inline void logUdpFrame(struct udphdr *udp, struct packet *pkt, void *data_end)
{
  struct packet_frame *frame = getFrame(pkt);
  if (!frame)
  {
...
__attribute__((__always_inline__)) static inline void logIcmp6Frame(struct icmp6hdr *icmp6, struct packet *pkt, void *data_end)
{
  struct packet_frame *frame = getFrame(pkt);
  if (!frame)
  {
...
__attribute__((__always_inline__)) static inline void logIpFrame(struct iphdr *ip, struct packet *pkt, void *data_end)
{
  struct packet_frame *frame = getFrame(pkt);
  if (!frame)
  {
...
__attribute__((__always_inline__)) static inline void logIpFrameHeader(struct iphdr *ip, struct packet *pkt)
{
  struct packet_frame *frame = getFrame(pkt);
  if (!frame)
  {
...
__attribute__((__always_inline__)) static inline void logIp6Frame(struct ipv6hdr *ip6, struct packet *pkt, void *data_end)
{
  struct packet_frame *frame = getFrame(pkt);
  if (!frame)
  {
...
__attribute__((__always_inline__)) static inline void logIp6FrameHeader(struct ipv6hdr *ip6, struct packet *pkt)
{
  struct packet_frame *frame = getFrame(pkt);
  if (!frame)
  {
...
__attribute__((__always_inline__)) static inline void logEthFrame(struct ethhdr *eth, struct packet *pkt, void *data_end)
{
  struct packet_frame *frame = getFrame(pkt);
  if (!frame)
  {
...
__attribute__((__always_inline__)) static inline void logEthFrameShallow(struct ethhdr *eth, struct packet *pkt, void *data_end)
{
  struct packet_frame *frame = getFrame(pkt);
  if (!frame)
  {
...
__attribute__((__always_inline__)) static inline void logXDP(struct xdp_md *ctx)
{
...
__attribute__((__always_inline__)) static inline void logPacketRedirectIfIdx(__u32 ifidx)
{
...
__attribute__((__always_inline__)) static inline void setBufferOnPacket(__u8 *data, __u32 data_len)
{
  if (data_len <= 32)
  {
    struct packet *pkt = getPacket();
