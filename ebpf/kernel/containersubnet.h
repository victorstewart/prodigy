...
__attribute__((__always_inline__)) static inline int redirectToContainer(__be8 *addr6, bool is_ingress) // addr6 is the entire ipv6 destination address of the packet
{
...
__attribute__((__always_inline__)) static inline bool isULA(__be8 *addr6)
{
  return (addr6[0] == 0xfd);
}

__attribute__((__always_inline__)) static inline bool isMulticast(__be8 *addr6)
{
  return (addr6[0] == 0xff);
}
