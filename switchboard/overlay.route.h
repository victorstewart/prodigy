...
{
  return lhs.prefixlen == rhs.prefixlen && std::memcmp(lhs.addr, rhs.addr, sizeof(lhs.addr)) == 0;
}
...
{
  switchboardBuildPrefixKeys(prefixes, ipv4Keys, ipv6Keys, [](const IPPrefix& prefix) -> switchboard_overlay_prefix4_key {
...
{
  return lhs.family == rhs.family && lhs.use_gateway_mac == rhs.use_gateway_mac && lhs.next_hop4 == rhs.next_hop4 && lhs.source4 == rhs.source4 && std::memcmp(lhs.next_hop_mac, rhs.next_hop_mac, sizeof(lhs.next_hop_mac)) == 0 && std::memcmp(lhs.next_hop6, rhs.next_hop6, sizeof(lhs.next_hop6)) == 0 && std::memcmp(lhs.source6, rhs.source6, sizeof(lhs.source6)) == 0;
}
...
static inline bool switchboardParseMACLiteral(const String& text, uint8_t bytes[6])
{
  ...
  for (uint32_t index = 0; index < 6; ++index)
  {
    ...
    uint8_t high = 0;
    uint8_t low = 0;
    if (hexNibble(char(text[offset]), high) == false || hexNibble(char(text[offset + 1]), low) == false)
    {
      return false;
    }
