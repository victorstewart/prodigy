...
__attribute__((__always_inline__)) static inline bool containsContainerNetworkIPv6(const __u32 dstv6[4])
{
  ...
  return bytes[0] == container_network_subnet6.value[0] && bytes[1] == container_network_subnet6.value[1] && bytes[2] == container_network_subnet6.value[2] && bytes[3] == container_network_subnet6.value[3] && bytes[4] == container_network_subnet6.value[4] && bytes[5] == container_network_subnet6.value[5] && bytes[6] == container_network_subnet6.value[6] && bytes[7] == container_network_subnet6.value[7] && bytes[8] == container_network_subnet6.value[8] && bytes[9] == container_network_subnet6.value[9] && bytes[10] == container_network_subnet6.value[10];
}

__attribute__((__always_inline__)) static inline bool ownedRoutablePrefixesContainIPv4(__be32 dest_ip)
{
...
__attribute__((__always_inline__)) static inline struct switchboard_overlay_machine_route *balancerLookupOverlayMachineRouteForContainer(struct container_id *containerID)
{
  ...
  __u32 machine_fragment = ((__u32)containerID->value[1] << 16) | ((__u32)containerID->value[2] << 8) | (__u32)containerID->value[3];
...
__attribute__((__always_inline__)) static inline bool set_container_id_from_routable_ipv6(struct container_id *containerID, const __u32 dstv6[4], const struct local_container_subnet6 *localcontainersubnet6)
{
  const __u8 *bytes = (const __u8 *)dstv6;
...
__attribute__((__always_inline__)) static inline int process_l3_headers(struct packet_description *pckt, __u64 off, __u16 *packet_len, void *data, void *data_end, bool is_ipv6)
{
  if (is_ipv6)
  {
  ...
  else
  {
    ...
    {
      return XDP_DROP; // drop fragments
    }
  }
...
__attribute__((__always_inline__)) static inline int process_packet(struct xdp_md *xdp, __u64 off, bool is_ipv6)
{
  ...
  if (is_ipv6)
  {
    bpf_memcpy(portal.addr6, pckt.flow.dstv6, 16);
  }
  else
  {
