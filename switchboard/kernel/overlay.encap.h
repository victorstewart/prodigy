...
__attribute__((__always_inline__))
static inline bool switchboardMaybeRouteHostedIngressIPv4(struct __sk_buff *skb, struct ethhdr *eth, void *data_end, int *action)
{
   if (action == NULL)
   {
      return false;
...
__attribute__((__always_inline__))
static inline bool switchboardMaybeRouteHostedIngressIPv6(struct __sk_buff *skb, struct ethhdr *eth, void *data_end, int *action)
{
   if (action == NULL)
   {
      return false;
