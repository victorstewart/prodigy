...
__attribute__((__always_inline__)) static inline bool accumulate_packet_checksum_bytes(__u32 *csum, const void *value, __u16 byteCount, const void *data_end)
{
  ...
  if (boundedByteCount > SWITCHBOARD_MAX_WORMHOLE_CHECKSUM_BYTES || (const void *)(bytes + boundedByteCount) > data_end)
  {
    return false;
  }
...
__attribute__((__always_inline__)) static inline bool accumulate_fixed_checksum_words(__u32 *csum, const void *value, __u32 byteCount)
{
...
__attribute__((__always_inline__)) static inline bool accumulate_skb_checksum_bytes(struct __sk_buff *skb, __u32 *csum, __u32 byteOffset, __u16 byteCount)
{
  __u16 boundedByteCount = byteCount;
...
__attribute__((__always_inline__)) static inline bool compute_ipv6_transport_checksum_skb(
    __u16 *checksumOut,
    ...
    __u8 nextHeader)
{
  ...
  __u32 csum = 0;
  if (accumulate_fixed_checksum_words(&csum, srcv6, 16) == false || accumulate_fixed_checksum_words(&csum, dstv6, 16) == false || accumulate_fixed_checksum_words(&csum, lengthBytes, sizeof(lengthBytes)) == false || accumulate_fixed_checksum_words(&csum, nextHeaderBytes, sizeof(nextHeaderBytes)) == false)
  {
    return false;
  }
  ...
  if (suffixBytes > 0 && accumulate_skb_checksum_bytes(
                             skb,
                             &csum,
...
__attribute__((__always_inline__)) static inline bool compute_ipv6_transport_checksum_in_packet(
    __u16 *checksumOut,
    const void *segment,
    __u16 segmentSize,
    __u16 checksumByteOffset,
    ...
    const void *data_end)
{
  ...
  if (boundedSegmentSize > SWITCHBOARD_MAX_WORMHOLE_CHECKSUM_BYTES || (const void *)((const __u8 *)segment + boundedSegmentSize) > data_end)
  {
    return false;
  }
  ...
  if (suffixBytes > 0 && accumulate_packet_checksum_bytes(
                             &csum,
...
__attribute__((__always_inline__)) static inline bool recompute_ipv6_transport_checksum_in_packet(
    __u16 *checksumField,
    const void *segment,
    __u16 segmentSize,
    __u16 checksumByteOffset,
...
{
  __u16 recomputed = 0;
  if (checksumField == NULL || compute_ipv6_transport_checksum_in_packet(
                                   &recomputed,
                                   segment,
                                   segmentSize,
                                   checksumByteOffset,
...
__attribute__((__always_inline__)) static inline bool store_recomputed_ipv6_transport_checksum_skb(struct __sk_buff *skb, __u8 nextHeader)
{
  void *data = (void *)(long)skb->data;
  void *data_end = (void *)(long)skb->data_end;
  struct switchboard_ipv6_skb_layout layout = {};
  struct ipv6hdr ip6h = {};
  if (switchboardResolveIPv6SKBLayout(data, data_end, skb->protocol, &layout) == false || bpf_skb_load_bytes(skb, layout.l3Offset, &ip6h, sizeof(ip6h)) != 0)
  {
    return false;
  }
